...
total=$(ask 'SELECT (COUNT(DISTINCT ?s) AS ?n) WHERE { ?s <https://pwg.dev/ontology#displayName> ?v }')
alive=$(ask 'SELECT (COUNT(DISTINCT ?s) AS ?n) WHERE { ?s <https://pwg.dev/ontology#displayName> ?v . FILTER(REGEX(STR(?v), "[a-z]", "i")) }')
never=$(ask 'SELECT (COUNT(DISTINCT ?s) AS ?n) WHERE { ?s <https://pwg.dev/ontology#displayName> ?v . FILTER(REGEX(STR(?v), "^zzqqxxnotaword\\b", "i")) }')
store=$(ask 'SELECT (COUNT(DISTINCT ?s) AS ?n) WHERE { ?s <https://pwg.dev/ontology#displayName> ?v . FILTER(REGEX(STR(?v), "^(mum|mummy|mother|dad|daddy|father|wife|husband|gran|granny|grandma|grandad|grandpa|nan|nana|auntie|aunt|uncle|bro|sis|sister|brother|son|daughter|cousin|in-law)\\b", "i")) }')
