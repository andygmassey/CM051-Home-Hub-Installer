...
| v1.0.69 |  | not_walked | Archie 2026-09-05. **TAGGED AT be221728 AND NEVER BUILT.** The cut workflow refused it at PREFLIGHT, twice, on walk closure -- and the refusal was the GATE'S OWN BUG, not a missing row. The row lookup piped its input into an awk that exits on the first match; awk closed the pipe, printf took EPIPE, `pipefail` promoted it, and eight rows that are demonstrably present in the register were reported NO ROW. Every refusal printed its own cause: `line 322: printf: write error: Broken pipe`. 🔴 IT DID NOT REPRODUCE ON macOS NOR IN AN ubuntu:24.04 CONTAINER running the same gate against the same registry, both GREEN -- the rows blob is ~55KB against a 64KB pipe buffer, so the writer normally finishes first. THE DEFECT WAS LATENT AND GROWING: the register gains a row per cut, so it would have begun refusing releases intermittently with no code change to blame. Fixed in OS003 #220 with a here-string and a 5-arm test that builds a 422KB blob rather than trusting the real registry, because a test against the real file would pass today and start failing later. ⚠️ NO ARTEFACT EXISTS for v1.0.69, so there is no walks/v1.0.69.tsv and there should not be. Its payload -- #1537 the cold-store cap, #1520 diagnostics persistence, #1515 the killed-converge marker -- is carried forward unchanged by v1.0.70. 📌 THE PIN WAS ALSO RE-POINTED MID-CUT and that is recorded because it nearly shipped wrong: v1.0.69 was pinned at 7cd88739, which PREDATES #1537, so it would have shipped without the fix it existed for. `merge-base --is-ancestor d996b999 7cd88739` returned NO and caught it. @ANDY: nothing was installed from this tag. | none |
| v1.0.70 |  | not_walked | Archie 2026-09-05. **TAGGED AT f443571c AND NEVER BUILT. The cut job died in nine seconds on the ONE pre-cut gate whose input is not a tree.** `[RED] CM051:#1549` at step 8. Every other check reads a repository, which freezes the instant the tag exists; the orphan gate reads the set of OPEN PULL REQUESTS, and that set is mutable by anyone at any moment. 🔴 **I READ IT TEN MINUTES EARLY.** `gh pr list --state open` at 13:10Z returned three, all deferred or mine. TNM opened #1549 at 13:14:22Z. I pushed the tag at 13:20:01Z. The gate did exactly its job and the mistake is mine, not the gate's. The rule that follows is narrow and I am keeping it: **re-read the open-PR list in the same minute as the tag push**, and treat the gap between that read and the push as the window I own. It is NOT 'ask people to stop opening PRs' -- holding work back to keep a gate quiet is the habit this gate exists to punish. 🟢 **EVERYTHING BEFORE STEP 8 WAS GREEN**: preflight success, the orphan gate's own 3-proof self-test on /bin/bash 3.2 success, pre-cut gates 15/0 with rc taken directly, and the pin `CM051=2d9dbaed` verified by BLOB rather than ancestry (`install.sh` at the pin and at f443571c are the same object b667ff7d) after each of four merges. ⚠️ **TWO GATE REDS ON THE FIRST PREFLIGHT RUN WERE MY ENVIRONMENT AND ONE IS A HOLE IN A GATE**, filed as CM051 #1550: the wiki-content gate guards `CM044_DIR` against being a WORKTREE but not against a canonical CLONE on a stale branch, which is the same failure with the same consequence. The checkout was 7 commits behind main on a feature branch; against a fresh clone at main the same gate passes with its own positive controls firing, and the wiki compiler pin is byte-identical across v1.0.67, v1.0.68 and v1.0.70. A stale checkout means the gate COULD NOT LOOK, which is CANNOT-RUN and not RED -- and the two drive opposite actions, because a RED sends someone to rebuild and re-pin an image that is already correct. 🔴 **AND #1549 IS WHY THIS FAILURE WAS WORTH HAVING.** I had written, here and in the handoff, that the proof #1543 needs is 'a fresh install on a virgin account reading 0'. TNM's PR refutes that: a virgin account whose contacts share no phone or email gives `resolve()` nothing to match, the RULE 2 guard is never asked to act, and the probe reports a clean pass with a healthy control. The old anti-vacuity control proves the PREDICATE CAN SEE; it does not prove the BOX PRESENTED THE CONDITIONS. **v1.0.70 would have produced exactly that vacuous zero and I would have closed a launch blocker on it.** ⚠️ NO ARTEFACT EXISTS for v1.0.70, so there is no walks/v1.0.70.tsv and there should not be. Its payload -- #1537 cold-store cap, #1520 diagnostics, #1515 killed-converge marker, #1543 RULE 2 on the write -- carries forward unchanged to v1.0.71, which also picks up #1549's three-state EARNED/UNEXERCISED/UNMEASURED verdict, without which the walk cannot prove #1543. @ANDY: nothing was installed from this tag. | none |
| v1.0.71 |  | deferred | Archie 2026-09-07, AMENDING MY OWN ROW OF 2026-09-06. **not_walked -> deferred, BECAUSE I MADE THIS ROW'S OWN PREMISE FALSE.** The text below says "There is NO `walks/v1.0.71.tsv`, so no harness record exists and the status is read as typed." That was true when written. On 2026-09-07 I found TWO records sitting UNCOMMITTED in an operator worktree, one under each spelling -- `walks/1.0.71.tsv` at 16:05:06Z (pass 17 fail 7 cannot_run 1) and `walks/v1.0.71.tsv` at 16:18:47Z (pass 19 fail 5 cannot_run 1), same box_fp 95b30a89637eecbb, 13 minutes apart, both verdict FAILED -- and committed the v-prefixed one in CM051 #1790 WITHOUT amending this row in the same change. The next cut refused at preflight: `v1.0.71 row says not_walked but a walk record exists with verdict FAILED`, WALK CLOSURE RED, one cut unaccounted for. The gate is right and the disagreement was mine: a register and a record that contradict each other by one row is exactly what it exists to catch, and it caught it in nine minutes. **DEFERRED, NOT CLOSED, AND THE ORIGINAL REASONING BELOW STILL BINDS.** `artefact_sha256` is `unavailable` in both records, with `artefact_sha256_source` reading `asserted-unverifiable(no OstlerInstaller-1.0.71.dmg on the box)`. So these say a box was walked at something calling itself 1.0.71; they do NOT establish it was the published artefact, which is the very test the text below applied to refuse counting the green install. The status changes because a RECORD now exists, not because attribution improved. **THE TWO FILENAMES ARE THEMSELVES THE FINDING**: a gate asking for the v-prefixed spelling cannot see `walks/1.0.71.tsv`, which is the defect CM051:archie/land-1744-either-spelling fixes -- and that branch was local-only until 2026-09-07T10:50Z, so the bug and its evidence were invisible for the same reason. Only the v-prefixed record is committed; all nine existing records use that spelling and a non-v file would be the first. **THIS DOES NOT CREATE A GREEN WALK.** It moves v1.0.71 from unwalked to walked-twice-failed-twice. --- ORIGINAL ROW, 2026-09-06, UNCHANGED: --- Archie 2026-09-06. **TAGGED, CUT, PUBLISHED, AND NOT WALKED BY THE HARNESS.** The cut run concluded success 2026-09-05T13:58Z and the artefact was published to `ostler-ai/ostler-installer` at 2026-09-05T14:10:36Z with 3 assets. I checked all three release repos before writing this, because probing the wrong one returns a 404 that reads as never-built: CM051 and `ostler-releases` have no v1.0.71 release, `ostler-installer` does, and a control listing that repo returns v1.0.71/.68/.67/.66/.65. There is NO `walks/v1.0.71.tsv`, so no harness record exists and the status is read as typed. **AN INSTALL DID RUN AND WENT GREEN, AND I AM NOT COUNTING IT AS A WALK.** On archie@ the last install ended `DONE status=ok failed_steps=0 errors=0` with diagnostics at `20260905T151850Z`, 68 minutes after publication. That timing is suggestive and it is not evidence: the install records NO build identifier (`~/.ostler/daemon-build.json` carries `version: null`, and `grep` for a `1.0.NN` string across the whole install log returns nothing), so the artefact cannot be attributed to that run by anything except the clock. A walk that cannot be attributed to a build is not a walk of that build. THIS ROW EXISTS BECAUSE ITS ABSENCE BLOCKED THE v1.0.72 CUT AT PREFLIGHT, which is the gate working: I declared the cut gates green on the five I knew to run and the walk-closure gate was not among them. The denominator was the gates I knew, not the gates that decide. | none |
| v1.0.72 |  | not_walked | Archie 2026-09-07. **TAGGED AND THE CUT FAILED IN BUILD. NO ARTEFACT WAS PUBLISHED, so there is no walks/v1.0.72.tsv and there should not be one.** The shipping ledger records this version as TAGGED_CUT_FAILED_IN_BUILD_NO_ARTEFACT_PUBLISHED with box_walked false. Nothing was signed, nothing was uploaded, nobody installed it. ⚠️ **DO NOT CONFUSE THIS WITH THE 1.0.72-STAMPED DMG THAT DOES EXIST.** A separate emergency walk-only build carrying CFBundleShortVersionString 1.0.72, sha256 6f001449be3b6637d9b2bc454cd0fb03ac31553b41f0c133718c42e5288ec658, was cut by hand on 2026-09-07 and is filed in the ledger under its own row `1.0.72-walk-2026-09-07`, tag NONE. It was never walked either. Two different objects share a version STRING and only one of them was ever tagged, which is exactly the artefact-versus-tag confusion this register exists to keep straight: tell them apart by DIGEST, never by filename or stamp. 📌 This row exists because its absence, together with v1.0.73's, refused the v1.0.74 cut at preflight with `WALK CLOSURE RED -- 2 cut(s) in [v1.0.41, v1.0.74) are not accounted for`. The gate was right and the cost was a spent tag. | none |
| v1.0.73 |  | deferred | Archie 2026-09-07. **BUILT, SIGNED, NOTARISED, STAPLED, NEVER TAGGED, AND WALKED THREE TIMES BY ANDY AT THE CONSOLE. ALL THREE ABORTED.** Ledger status BUILT_SIGNED_NOTARISED_STAPLED_WALK_ONLY_NOT_DISTRIBUTED, built_from_commit c0401242, artefact OstlerInstaller-1.0.73.dmg sha256 963cec504270ac1fa4b8330d1202ddad884ed8a9da8200643b09b9bef6f24b64. **There is NO walks/v1.0.73.tsv and there should not be**: post_walk_qa writes a record after an install, and no install ever completed, so the absence is correct rather than an omission. 🔴 **ATTEMPTS 1 AND 2 DIED AT STEP 5 OF 41 (12%), ATTEMPT 3 AT STEP 6 (14%).** Two distinct defects, one class: a status that is expected and harmless made fatal by the ERR trap at install.sh:10793. install.sh:12399 called `_ollama_agent_is_running` as a BARE command, and its return 1 -- agent registered but not yet `state = running` -- fired the trap; the 90-second poll directly below it, which exists to absorb exactly that window, never ran one iteration, which is what `elapsed_s=0` recorded on both attempts. install.sh:13022 ran `tr -dc <alphabet> < /dev/urandom \| head -c 20`, where head exits and closes the pipe, tr dies on SIGPIPE and pipefail promotes it to 141; the loud ERR-14 credential guard three lines below was unreachable for the same reason. Reproduced 10/10 against the pre-fix construct, so deterministic and not a race. ⚠️ **THE PORT COLLISION FIRST BLAMED FOR ATTEMPT 1 WAS NOT THE CAUSE.** A stale com.ostler.ollama did hold 11434 and ollama.err did carry real bind errors, but attempt 2 started Ollama cleanly -- pid bound, /api/tags 200, our own label -- and aborted at the identical line. The bind errors read from were the OLD ones already in the file. ⚠️ **AND IT IS THE TRAP, NOT ERREXIT.** Errexit is OFF at :12399 (:8140 `set -uo pipefail`); an ERR trap fires either way, measured on two arms differing only in -e. `set +e` would not have helped; only `trap - ERR` does, which is why all 7 disarm sites in install.sh pair the two. 📌 Both defects are filed as CM051 #1754 and FIXED on main, riding v1.0.74, whose payload was then driven to 41 of 41 steps with no errors raised. **DEFERRED, NOT CLOSED**, for the standing reason (#931): the fixes are in the repo and were in NO artefact at the time this row was written, so closing would be true about the repo and false about the DMG. @ANDY walked this one himself; correct anything not captured. | none |
| v1.0.74 | 2026-09-07 | deferred | Archie 2026-09-07. **TAGGED, CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED, AND THIS IS THE FIRST v1.0.7x THAT INSTALLED TO DONE ON A COLD BOX.** Artefact OstlerInstaller-1.0.74.dmg sha256 9d122025ca38261469a53c9748509741867ba2786288b57acfa3fbb9271854a8, tag commit a340ce91, cut run 34121388690, ONE tag push. The thin walk drove the payload through 39 of 39 steps: STEP_END not-ok 0, status=error 0, status=timeout 0, ERR-NN codes 0, tracebacks 0, abort/fatal 0, exit 0, terminal marker `#OSTLER DONE status=ok failed_steps=0 errors=0`. **THE v1.0.73 STEP-5 ABORT IS THEREFORE CLOSED IN AN ARTEFACT, not merely on main**, which is what the v1.0.73 row above was deferred waiting for. 📌 **THE WALK RECORD EXISTS AND ITS VERDICT IS FAILED**: `walks/v1.0.74.tsv` (CM051 #1798), probes 20 pass / 4 fail / 2 cannot-run of 26, and `scripts/verify_walk_record.sh` correctly refuses the promote (rc=1) on three artefact-owned probes. This row is `deferred` and NOT `not_walked` for exactly the reason the v1.0.71 row had to be amended: a record exists, so claiming the cut was never walked contradicts it and refuses the next cut at preflight. ⚠️ **READ THE COUNTS WITH THE COLD-BOX CAVEATS.** stores_provenance was carried-over-from-previous-install (the colima VM was deleted and verified gone, so --wipe-stores had nothing to wipe and correctly refused, rc=2, and the walk ran --reset alone), so every graph-reading or vector-reading probe measured HISTORY rather than this artefact. The iMessage Automation probe was SHIMMED to check-failed because macOS TCC needs a GUI click ssh cannot make, so the real automation probe was NOT tested. The install-time converge was SIGKILLed at its 300s budget (killed_at_utc 2026-09-07T12:51:53Z) and the dedupe catch-up agent was STILL RUNNING while the people probes measured, so both people verdicts are mid-flight readings rather than end-state ones. 🔴 **THE BLOCKING GROUNDED PROBE'S ROOT CAUSE WAS FOUND ON THIS WALK AND IT IS NOT THE MODEL.** A person seeded through the product's own API had her employer returned verbatim by both /people/context and /api/v1/people/{slug}/enrichment, while pwg_people handed the model only a name and a wiki link, so the assistant accurately reported a payload that had been stripped. render_person in crates/zeroclaw-tools/src/pwg_people.rs projected name/organisation/title/last_contact/identifiers/wiki_url and never read `facts`. Fixed in ostler-assistant #386. A second defect found on the same walk: consolidation wrote a durable Daily memory for EVERY turn including turns whose content was that a lookup failed, and the assistant then recalled its own prior non-answers (4 of 5 recalled memories, top-ranked 86%) and answered from them instead of querying the graph. Fixed in ostler-assistant #387. **BOTH ARE DAEMON-SIDE AND IN NO ARTEFACT**, so they need one hub-v build, one pin and one cut to reach a DMG. **DEFERRED, NOT CLOSED**, for the standing reason (#931): the fixes are in the repo and in no artefact, so closing would be true about the repo and false about the DMG. Closes when a v1.0.75 walk re-seeds and the grounded probe moves. | none |
| v1.0.76 | 2026-09-08 | deferred | Archie 2026-09-08. **TAGGED TWICE, CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED -- AND DEAD.** Artefact OstlerInstaller-1.0.76.dmg sha256 ba9ff4a743781d2dca6e30485a1f5aaaea4f71f35db3bd2ee3dcb14421fe2186, tag commit 91577dce, cut run 34204953380, all 15 cut steps green. Verified twice on the DOWNLOADED bytes, mine and Aesop's independently: stapler validates, spctl accepts source=Notarized Developer ID, and on-mount parity daemon = wrapper = e3ddf5de. 📌 **THERE IS NO walks/v1.0.76.tsv AND THERE SHOULD NOT BE**, the same shape as the v1.0.73 row above: post_walk_qa writes a record after an install, and on the genuinely cold box no install ever completed, so the absence is correct rather than an omission. 🔴 **WHY IT IS DEAD: install.sh SHIPS INSIDE THE DMG AND CARRIES A FIRST-INSTALL DEADLOCK.** graph_db_start preflights 6333 and 7878, this user's own colima multiplexer holds them, and #1253 signal 2 demanded the store ANSWER our credential -- but that step is the one that STARTS the store. A container left running by an earlier walk answered 401 to the freshly written credential, so the check refused the install because the store held the wrong key and blocked the only step that rewrites it. ERR-06-PORTS-HELD, rc=1, DONE status=fail. No re-walk of this artefact can ever produce a clean record; only a re-cut can. Fixed by CM051 #1835 and carried by v1.0.77. ⚠️ **THE FIRST v1.0.76 TAG WAS SPENT AND THE MACHINERY WAS THE BUG**, exactly as item 4 says a second push means. Run 34201583498 died at preflight in about forty seconds with no artefact, on two hard gates that scripts/new_cut.sh could not see while printing ALL GREEN, safe to tag: the missing cut-manifests/v1.0.76.yaml and an installer still stamped 1.0.75/7500. Fixed in #1825 BEFORE the re-tag rather than beside it. Re-using the number was checked, not assumed: gh release view returned release not found, so no artefact existed under it and it cannot mean two things. 🟢 **THIS CUT IS WHY THE WALK WORKS AT ALL NOW.** Its cold walk was the first honestly cold one ever run, because --reset had never uninstalled (#1828, fixed by #1829), and it found four defects in a day: that teardown gap, people_count adjudicating a moving graph (#1830), the new_cut.sh blindness (#1825), and the port deadlock (#1835). None was in the artefact's content and none was findable before. **DEFERRED, NOT CLOSED**, for the standing reason (#931): every fix above is in the repo and in NO artefact, because v1.0.76 was cut from 91577dce which predates all of them. Closes when a v1.0.77 walk moves the blocking probes. | none |
| v1.0.75 | 2026-09-08 | deferred | Archie 2026-09-08. **TAGGED, CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED, AND WALKED ON A GENUINELY COLD BOX.** Artefact OstlerInstaller-1.0.75.dmg sha256 0acfab4140f66c29647b767f5bb45fe5febf2f8793a34a6b742978a0c5cbb011, tag commit 8dc8c104, cut run 34156581655. **THIS WAS THE SIXTH TAG UNDER ONE VERSION**: five earlier pushes died before producing any artefact, each with a DIFFERENT root cause and none a repeat, and all six are recorded in the HR015 ledger under the v1.0.75 dmg_cuts attempts list. Item 4 says a second push means the cut machinery is the bug; five means it was five separate bugs, each only reachable once the previous one cleared. The install itself reached DONE status=ok, 0 failed steps, exit 0, 38 of 38 steps. 📌 **THE WALK RECORD EXISTS AND ITS VERDICT IS FAILED**: walks/v1.0.75.tsv, phase-1 probes 18 pass / 6 fail / 2 cannot-run of 26. This row is deferred and NOT not_walked, for the same reason the v1.0.71 row had to be amended: a record exists, so claiming the cut was never walked contradicts it and refuses the next cut at preflight. 🟢 **b4118b45 IS PROVEN AT THE REPLY HOP**, the first seeded grounded turn on any artefact: the seeded question scored grounded, which is a CONTENT verdict rather than a frame-shape one, so the chain memory-assert to PersonFact to people-context facts to pwg_people to the reply is proven end to end. 🔴 **THE WALK'S REAL PRIZE WAS CM051 #1820**: _ostler_ensure_qdrant_collections had exactly ONE call site and it sat INSIDE the GDPR-import branch, so a customer who declines the import on day one never reached #606's pre-creation and finished with conversations and evernote_knowledge absent. Not cold-box-specific, which was the first hypothesis and wrong; IMPORT-DECLINED-specific. Fixed by #1821 and box-demonstrated 404 to 200. 🔴 **Q1 WAS DIAGNOSED AND IT IS NOT ROUTING.** Asked what it knew about the customer, the assistant made ZERO tool calls and recited a memorised count of 1859 contacts while the live graph held 1882. Control, same box and same instant with wording the only variable: novel wording called pwg_overview and returned the correct 1882, the memorised wording returned the stale 1859. So the tool ships, the instruction ships, and the fault is memory-vs-retrieval arbitration. It self-poisoned across runs, and both halves were already in this walk's own record before anyone read them that way: Q1 scored grounded at 03:45:48Z and no_tool_call at 04:31:45Z, after its own answers had been memorised. Fixed write-side in ostler-assistant #388. ⚠️ **people_count_agreement WAS A SAMPLING ARTEFACT, NOT A STORE DEFECT.** The converge was SIGKILLed at 03:25:09Z and its catch-up finished at 03:49:25Z, while post_walk_qa ran 03:29 to 03:45, entirely inside that window. Re-measured after settling: oxigraph and doctor agree at 1882 exactly across six paired samples, and the 13 mergedInto tombstones are present in BOTH surfaces, so it is one population. The 300s budget is deliberate, so the box behaved as designed and the probe fired mid-catch-up. Fixed in #1822. **DEFERRED, NOT CLOSED**, for the standing reason (#931): every fix above is in the repo and in NO artefact, because v1.0.75 was cut from 8dc8c104 which predates all of them. Closes when a v1.0.76 walk moves the blocking probes. | none |
| v1.0.77 |  | not_walked | Archie 2026-09-09. **TAGGED ONCE AT a46763a6 AND NEVER BUILT.** Cut run 34250843633 concluded failure at 2026-09-08T16:24:23Z in PREFLIGHT step 9, `BOM rows must be in the pinned tree`. The dry-run and cut jobs are both SKIPPED, so nothing was compiled, signed, notarised or uploaded. Checked all three release repos before writing this, because probing the wrong one returns a 404 that reads as never-built: no v1.0.77 release exists in CM051, in `ostler-ai/ostler-installer` or in `ostler-ai/ostler-releases`. **There is NO walks/v1.0.77.tsv and there should not be**, the same shape as the v1.0.73 and v1.0.76 rows: post_walk_qa writes a record after an install, no artefact ever existed, so the absence is correct rather than an omission. 🟢 **ONE TAG PUSH, WHICH IS WHAT ITEM 4 ASKS FOR.** cut.yml carries exactly one run on branch v1.0.77. ⚠️ **THE REFUSAL WAS EXIT 2, CANNOT-RUN, NOT A FAILING BOM.** The gate's own words: `not one row was checkable against the pin, so '0 absent' here means 'nothing was examined'`. Rows 1, in the pin 0, ABSENT 0, unmeasurable 1, against pin 9e380700 (install.sh, 31976 lines). Had it graded that as a pass it would have shipped an unexamined BOM under a green tick, so the gate did the right thing with the answer it had. 🔴 **THE BLIND SPOT WAS REMOVAL-SHAPED ROWS.** The single row was CM051#1835, the first-install port deadlock fix that killed v1.0.76, and that fix DELETES lines from install.sh and adds no keyable line. The gate keyed presence only, so a fix that ships by subtraction was unmeasurable by construction, and every future removal-only fix would have spent a tag the same way. Fixed by CM051 #1841, which keys a removal by asserting the DELETED line is ABSENT from the pinned blob. Control on the same inputs, same host, gate the only variable: the origin/main gate exits 2 with output byte-identical to the CI refusal, the fixed gate exits 0 and prints `IN THE PIN CM051#1835 by REMOVAL: the line it deleted is absent from the pinned install.sh`. #1841 also adds tests/test_new_cut_covers_every_pretag_gate.sh, which derives the gate list from cut.yml and fails if new_cut.sh does not invoke each one, so the class that spent the FIRST v1.0.76 tag (new_cut.sh printing ALL GREEN while a hard preflight gate was missing from its list) cannot recur silently. 📌 install.sh is unchanged from v1.0.77 to v1.0.78: blob 3c41f392 is identical at 9e380700, at 8260c230 and at main, so v1.0.78 re-cuts the SAME payload past a fixed gate and carries no new install surface. **not_walked, NOT deferred**, because no artefact exists to defer a judgement about. | none |
| v1.0.78 | 2026-09-08 | deferred | Archie 2026-09-09. **CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED, AND WALKED ON A GENUINELY COLD BOX -- THE FIRST v1.0.7x FOR WHICH THAT SENTENCE IS TRUE OF THE STORES AS WELL AS THE VM.** Artefact OstlerInstaller-1.0.78.dmg 71,297,948 bytes, sha256 6801704d4c1d459807df0cbaa72d2bc26eda17009aafba523aa9d66eb1084515, tag commit 27752dcd, cut run 34258815278, ONE tag push, preflight 20/20 and all cut steps green. 📌 **THE WALK RECORD EXISTS AND ITS VERDICT IS FAILED**: walks/v1.0.78.tsv (CM051 #1847), 18 pass / 6 fail / 2 cannot-run, 26 of 26 measured, 0 broken, every control red on bad input first. `deferred` and NOT `not_walked`, for the reason the v1.0.71 row had to be amended: a record exists, so claiming the cut was never walked contradicts it and refuses the next cut at preflight. verify_walk_record.sh correctly REFUSES the promote (rc 1) on artefact_sha256 `unavailable` -- no DMG on the box, so the record says asserted-unverifiable rather than pretending. 🟢 **THE INSTALL ABORT IS CLOSED IN AN ARTEFACT, WHICH IS WHY THIS CUT EXISTED.** #OSTLER DONE status=ok failed_steps=0 errors=0, install.sh rc 0, 39 steps, zero status=error and zero status=timeout, ttywalk exit 0, free space 85,971,112 to 70,128,664 KB so a real VM was built rather than reused. The v1.0.73-class step-5 abort and the v1.0.76 ERR-06-PORTS-HELD first-install deadlock are both PROVEN GONE IN A DMG, not merely fixed on main: CM051 #1835 is the fix and this is the first build that could ever demonstrate it, because v1.0.76 could not install and v1.0.77 never built. ⚠️ **DO NOT READ stores_provenance IN THIS RECORD AS A FACT. IT SAYS carried-over-from-previous-install AND THAT IS NOT WHAT HAPPENED.** The value is written at ttywalk.sh:848 in the ELSE branch of "did we find a shipped uninstaller", and --reset never finds one because it looks in three paths install.sh does not write, so EVERY --reset walk stamps carried-over whatever the stores are. This box had its colima VM deleted and verified gone by two readings 6m44s apart, and install.sh:17257 declares the stores as NAMED docker volumes with no host path, which live inside that VM. Corroborated independently on the box: every entry under ~/.ostler dated Sep 9, hydration observed live at 03:26. The stores were FRESH. Its sibling arm at ttywalk.sh:758 does it correctly, writing its claim only after counting volumes. **THAT CORRECTION IS HALF THE DIAGNOSIS AND NOT THE WHOLE OF IT.** The stores were fresh, so the six FAILs are readings of THIS artefact rather than of carried-over history, which is the first honest read anyone has had of them. It does NOT follow that all six are defects. ⚠️ **THE TWO PEOPLE PROBES ARE PROVISIONAL AND THE RECORD ITSELF SAYS SO.** converge_kill_is_recorded FAILING means the install-time converge was SIGKILLed at its 300s budget, and its whole purpose is to declare that people counts on this box are provisional. Measured after the walk: doctor moved 1629 to 1920 while oxigraph held 1822, so the delta did not narrow, it INVERTED, and doctor ended AHEAD by 98. A gap that changes sign under re-reading is a moving target, not a fixed shortfall. This is the v1.0.75 shape exactly, where I called a people_count disagreement THREE times when there was none, because dedupe-converge.done means the CATCH-UP finished and never that the graph stopped changing (#1822 now adjudicates on stability across consecutive reads for that reason). So people_count_agreement and people_stores_reconcile are NOT YET established as defects; they are single samples taken inside the post-SIGKILL catch-up window. What IS established is root A itself: the converge is being killed at its budget on a roughly 1800-person account, and everything downstream of that is unmeasurable until it is fixed. 🔴 **TWO OF THE FOUR ITEM-5 BLOCKING PROBES ARE RED**, so Andy was NOT asked to walk: assistant_answers_grounded (2 of 4 turns did not reach the data, on a seeded fixture verified verbatim in the log) and people_stores_reconcile (193 named persons unsearchable). no_store_port_is_tcp_reachable and no_person_holds_two_contact_cards both PASS. usage_journal_producers FAILS and is NOT blocking for v1.0 per item 5. Also red and not named by item 5: converge_kill_is_recorded (install-time converge SIGKILLed at its 300s budget on a ~1800-person account, so counts are provisional), install_manifest_complete and people_count_agreement (oxigraph 1822 vs doctor 1629, the SAME 193). 🔴 **install_manifest_complete IS A RECURRENCE, NOT A NEW DEFECT.** install.sh:29350-29361 already records this exact shape from the v1.0.60 walk: the store answering `preferences, people, safari_history` while `conversations` and `evernote_knowledge` are absent and one present collection was never in the creation loop. Tonight the same two are missing. #1821 added a SECOND call site (install.sh:19731 and :19750) and _OSTLER_REQUIRED_QDRANT_COLLECTIONS (:18355) declares all four, so the creation is reached and declared: the surviving hypothesis is created-and-did-not-take or created-then-lost, NOT never-called, and #1821 is incomplete rather than wrong. **DEFERRED, NOT CLOSED**, for the standing reason (#931): the fixes for the six FAILs are not in any artefact. Closes when a v1.0.79 walk moves the blocking probes. | none |
| v1.0.79 | 2026-09-08 | deferred | Archie 2026-09-09, AMENDING MY OWN ROW OF AN HOUR AGO WITH RECORD #2. **RECORD #2 IS NOW THE RECORD**: walks/v1.0.79.tsv is the re-run on the SAME installed box (CM051 #1855), instrument_rev d1e0cda0 against record #1's 0feaa2fd, same box_fp, same artefact -- artefact X judged by a FIXED instrument Y. 21 pass / 4 fail / 1 cannot-run, 26 of 26, 0 broken, verdict still FAILED. 🟢 **CM051 #1849 WITH #1852 IS PROVEN, AND THE PROOF IS THAT THE TWO PEOPLE PROBES ARE GRADED AT ALL.** They were CANNOT-RUN on record #1 because the wait could not open its credential file. The wait now HELD STABLE at 1921/1920 across five readings a minute apart, which is a measurement of a settled graph rather than a marker anyone declared. ingest_coverage CANNOT-RUN -> PASS (all 4 stores, 9 of 9 sources) and people_count_agreement CANNOT-RUN -> PASS (differ by 1, allowance 5). 🔴 **people_stores_reconcile CANNOT-RUN -> FAIL, AND THAT IS THE HARNESS WORKING.** #1849 gates on per-store STABILITY and never on the two stores AGREEING; an equality gate would only have let this probe run once its own assertion was already true, which is a fixture. So this is the FIRST REAL READING of this row on this version: C=1 named person unsearchable, and D=the wiki tile reads 1920 and matches NEITHER store while the graph reconciles 1921 against 1921. 📌 **THE FORECAST IS INTACT.** It named THREE FAILs before the tag -- grounded, converge kill, usage journal -- and all three are still FAIL. It put the people pair in a separate category, free to pass or fail on a settled pair, and they SPLIT. Four FAILs is that category resolving, not a fourth surprise. --- ORIGINAL ROW, RECORD #1, UNCHANGED BELOW: --- **CUT, PUBLISHED, DELIVERED AND WALKED ON A COLD BOX, AND THE FORECAST HELD.** Artefact OstlerInstaller-1.0.79.dmg 71,293,492 bytes sha256 cfeae4c7fea7532f3dce87d5e83b97f90f875ad84364e3d2d743aca79ff7bf9d, tag commit 27752dcd -> 0feaa2fd, ONE tag push, cut run 34278011456, preflight 20/20 and all cut steps green. Record walks/v1.0.79.tsv (CM051 #1853): 19 pass / 3 fail / 4 cannot-run, 26 of 26 measured, 0 broken. `deferred` and NOT `not_walked`, for the reason the v1.0.71 row had to be amended: a record exists. **THE VERSION STAYS OPEN**: record #2 re-runs the two people probes on the same installed box after CM051 #1852, and its counts belong in this row. 🟢 **THE PREDICTION WAS WRITTEN BEFORE THE TAG AND IT HELD.** cut-manifests/v1.0.79.yaml and HR015 #856 named THREE expected FAILs and only three: assistant_answers_grounded (daemon), converge_kill_is_recorded (product, the flat 300s budget) and usage_journal_producers (non-blocking under item 5, still a FAIL row). The record carries exactly those. An interim phase-1 count of 4 was relayed mid-run; the record says 3 and the record is what counts. 🟢 **install_manifest_complete PASSED, WHICH IS THE PROVEN FOR CM051 #1848 AND #1850**, and it is the reason this cut existed. It FAILED on v1.0.78. Measured on the box's own install log with BOTH controls firing: `did not answer in time` 0 against v1.0.78's 1, `id=graph_db_start` 2 (control: the step IS covered), `STEP_BEGIN` 39 (control: the reader works), `could not prepare` 0 where v1.0.78 had it, curl rc=26 zero, DONE status=ok. ⚠️ **THE FIRST ZERO ONLY MEANS ANYTHING BECAUSE OF THE OTHER TWO.** My first attempt read walk_v1079.log, where all three score 0: that file is the DRIVER'S POLLING VIEW, 33 STEP_BEGIN samples against v1.0.78's 51, and it never sampled during graph_db_start. A zero there means the log does not cover the step, and publishing it would have been a false PROVEN. 🔴 **THE FOUR CANNOT-RUNs ARE HARNESS, NOT ARTEFACT**, and must not be read as four unknowns about the DMG. people_count_agreement and people_stores_reconcile were NOT MEASURED BECAUSE THE HARNESS READER COULD NOT OPEN ITS CREDENTIAL FILE: #1849's stability wait built the conf path as a literal $HOME and single-quoted it inside the ssh command, so the box never expanded it, curl exited 26, and both readings printed x once a minute for the whole budget. The console's stated reason, "the graph was still moving", is FALSE -- the stores were never read. Fixed by CM051 #1852 (d1e0cda0), which also un-gagged curl's stderr so the failure can name itself; the word for #1849 stays NOT PROVEN until record #2 shows a store read through it. app_signature_survives_first_run has no .app on an ssh-staged box; ingest_coverage is unchanged from v1.0.78. 🔴 **THE USAGE-JOURNAL ROW MEASURED THE STAGING-PATH JOURNAL BECAUSE THE WORKSPACE MARKER HOLDS THE PRE-PROMOTE PATH**, and the measurement is worse than two journals: there is only ONE and it is entirely under /tmp. Measured on the box as archie with controls in BOTH directions: ~/.ostler/active_workspace.toml reads `config_dir = "/tmp/ostler-prelaunch-50840/assistant-config"`; that journal holds 346 records; the FINAL path ~/.ostler/assistant-config/workspace/state/costs.jsonl IS ABSENT; and an invented path reads absent while a known path reads present, so the absence is absence and not a blind read. THREE FURTHER ORPHANED JOURNALS from earlier installs sit beside it holding 692, 1221 and 1938 records. So the customer's cost history accumulates under a prefix macOS clears on boot AND does not survive a reinstall. install.sh:13390 writes that marker in main flow BEFORE every main-flow call of _ostler_promote_prelaunch_tree and never rewrites it. **SIXTH INSTANCE of the class install.sh itself names at :353 and :13963** -- a value captured during staging and never rebound to the tree it will actually run from -- and the first of the six that is customer DATA rather than configuration. Present since before v1.0.68, bisect empty. 📌 artefact_sha256 reads unavailable: no DMG on the box, so the record says asserted-unverifiable. The artefact is ASSERTED here from the cut run and the ledger's release sha, not measured on the box; the ttywalk sha-binding gap is post-freeze. 📌 stores_provenance again says carried-over-from-previous-install and is again FALSE, the ELSE branch of a test for an uninstaller --reset can never find; the VM and its named volumes were deleted and verified gone before this install. **DEFERRED, NOT CLOSED**, for the standing reason (#931) and because record #2 is pending. | none |
| v1.0.80 |  | not_walked | Archie 2026-09-09. **TAGGED ONCE AT a467cc29 AND NEVER BUILT.** Cut run 34318524695 concluded failure at 2026-09-09T06:21:30Z. PREFLIGHT SUCCEEDED; the CUT job failed at step 8, `Build, sign, notarise, staple`, in eleven seconds. Steps 10, 11 and 12 then failed as CONSEQUENCES (`found no dist/*.dmg`), not independently, so this is ONE defect and not four. **There is NO walks/v1.0.80.tsv and there should not be**, the same shape as the v1.0.72, v1.0.73, v1.0.76 and v1.0.77 rows: post_walk_qa writes a record after an install, no artefact ever existed, so the absence is correct rather than an omission. Checked all three release repos before writing this, because probing the wrong one returns a 404 that reads as never-built: no v1.0.80 release exists in CM051, in `ostler-ai/ostler-installer` or in `ostler-ai/ostler-releases`. 🟢 **ONE TAG PUSH, WHICH IS WHAT ITEM 4 ASKS FOR.** cut.yml carries exactly one run on branch v1.0.80, and the tag was checked three-sided in the same minute as the push: tags/v1.0.80 absent local and remote, tags/v1.0.79 resolving so the endpoint demonstrably answers, tags/v1.0.99 absent as the control. 🔴 **THE CUT NEEDS TWO ARTEFACTS AND ONLY ONE WAS PUBLISHED.** The daemon tarball went to `ostler-ai/ostler-releases`, which `gui/Makefile:262` pins as DAEMON_REPO and which was correct. But `gui/Makefile:893` download-hub-app ALSO fetches Ostler.app from HUB_APP_REPO = `ostler-ai/ostler-assistant` at the SAME tag string, and there only a lightweight tag was created, never a release with assets. `DAEMON_RELEASE_TAG` names two different releases in two different repos, so "publish hub-v0.4.76" is ambiguous between them and was resolved one way only. 🔴 **AND A SECOND CAUSE THAT MAKES A RERUN IMPOSSIBLE.** `gui/Makefile:345` HUB_APP_SHA256 at the tag still read 1bad546c, the 0.4.75 bundle's digest, because the bump moved DAEMON_VERSION and left this digest behind. download-hub-app verifies fail-closed at :980-991, so publishing the 0.4.76 hub app only moves the failure from the FETCH to the CHECKSUM. The pin must move, that is a CM051 commit, and a tag cannot take one. **So v1.0.80 is SPENT and the work goes to v1.0.81** (CM051 #1863). ⚠️ **THE SWEEP THAT MISSED IT SEARCHED FOR THE OUTGOING DAEMON SHA AND VERSION**; a digest does not contain its version, so HUB_APP_SHA256 was outside that search by construction. `gui/Makefile:333-335` already says "Sweep for the outgoing SHA-256 as well as the outgoing version, or a bump silently lands half done" -- three lines above the pin, and read afterwards. 🔴 **NOTHING COULD HAVE CAUGHT IT, WHICH IS THE REAL FINDING.** No pre-tag gate asks whether the artefacts the build will FETCH exist: all seven new_cut.sh gates were green and correct. `scripts/pre_tag_live_checks.sh` exists, checks the daemon tarball live, is invoked by NEITHER new_cut.sh NOR cut.yml, and has zero hub-app coverage. And supply-chain-pins run 34317749743 printed "[skip] could not reach hub-v0.4.76 (auth/offline). NOT a pass: site 6 is unverified in this run" and then "9 passed, 0 failed", exit 0, GREEN -- `tests/test_daemon_pins_agree.sh:184-196` counts that branch as neither, so the one file that knows about the hub app is a control that cannot fail in CI, because a draft release 404s to the read token. Fix owned by TNM: the test fails on an unverifiable remote site, the workflow gets the read token cut.yml:883 already uses, and a pre-tag check that every artefact the build will FETCH is published, non-draft and readable, keyed on the Makefile's own variables so it cannot drift from what the build does. | none |
| v1.0.82 | 2026-09-09 | deferred | Archie 2026-09-09. **CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED, WALKED ON WIPED STORES, AND FAILED.** Artefact OstlerInstaller-1.0.82.dmg, sha256 d600ae17c78400ff12259b19e281bb1edf0caae2a98504218e1995f3cd6366b4, 71,307,331 bytes, tagged ONCE at efd03fcc9b28babeae28ace5948ab45fa5b12c27, cut run 34376906474 success. Record walks/v1.0.82.tsv: 20 pass / 6 fail / 0 cannot-run / 0 broken, 26 of 26, verdict FAILED, qa_exit 1, walked_at 2026-09-09T19:02:38Z, instrument_rev e54e5a3e, box_fp 95b30a89637eecbb. 🟢 **stores_provenance READS wiped-by-explicit-store-wipe. THIS IS THE FIRST WALK IN THIS PROJECT WHOSE STORES WERE ACTUALLY CLEAN**, and it is why the three store probes are attributable at all. Andy decided WIPE at 13:4xZ; the wipe ran the shipped uninstaller, took 5 ostler_ volumes to 0, and the licence was saved and restored (447 bytes) by CM051 #1883, which was written tonight BECAUSE the wipe destroyed it. 🟢 **cannot_run WENT 1 TO 0 AND THAT IS A FIRST TOO.** app_signature_survives_first_run has been CANNOT-RUN on every previous walk because nothing ever placed the .app: ttywalk stages the DMG's CONTENTS and never launches the bundle, and it says so at :393. Measured 17:07:52Z that /Applications/OstlerInstaller.app was ABSENT with Safari.app present as the control, so verify_walk_record.sh:172-176 (which refuses CLEAN on cannot_run != 0) made a CLEAN record structurally impossible. Step C3 placed the app and the probe PASSED, seal intact after a live import. **No ssh walk before tonight could ever have produced a CLEAN record.** 🔴 **THE SIX FAILS, AND THREE OF THEM ARE APPARATUS, NOT THE PRODUCT.** (1) install_error_honesty: the installer claimed a clean run while install.log carried 3 error lines in 1110. The probe is RIGHT, a summary is not a result. All three are the store-auth .pth firing before ~/.ostler/lib/ostler_store_auth.py exists at its promoted path (install.log:175, :492, :500) while every step reported status=ok. Effect cosmetic: all three interpreters import it after the promote, with a failing control. Risk not cosmetic: site.addpackage prints Remainder of file ignored, so a SECOND line would be skipped silently. FIXED for v1.0.83 by CM051 #1886. (2) ingest_coverage: reported com.ostler.fda-rerun 'dying on every fire'. FALSE. launchctl at 18:45:11Z: runs = 0, last exit code '(never exited)', StartInterval 3600 s against a QA twelve minutes after install. THE AGENT HAS NEVER FIRED. The probe's awk split '(never exited)' and read '(never' as the exit CODE. The FLAT half is a baseline file living on the RUNNER that survived the box wipe. Two probe defects; fixes in flight. **This row previously read as a product finding in the v1.0.83 manifest and that was my error, corrected there.** (3) assistant_answers_grounded: pwg_topics found nothing because the conversation seed could not run, and THAT is a seed defect, not the box: lib/conversation_seed.sh defaulted its settings path to an escaped $HOME, passed it through a single-quoting helper, and tested a literal path containing a dollar sign. The file WAS present on the box at 213 bytes. Its tilde branch was broken too, prepending $HOME instead of stripping. Fixed by CM051 #1885, which also caught a second instance TNM found: a silent credential-less curl at three sites including the forget. **The record must never be read as 'the box cannot run the conversation pipeline'.** 🔴 **THE THREE THAT ARE NOT APPARATUS.** pwg_preferences returned nothing even though the preference seed reported SEEDED AND ASSERTED on the same box in the same run. DIAGNOSED AND STRUCTURAL: the tool GETs /api/v1/preferences, which the vendored API serves read-only from a profile compiled HOURLY by the CM059 editor agent; at 18:49:11Z it returned interests [], count 0, generated_at 18:17:09Z, BEFORE the seed ran. No walk could ever have found the seed's rows through that tool. **And it is a customer-facing fact, not only a walk artefact: a fresh customer's assistant knows no preferences for up to an hour after install.** people_count_agreement and people_stores_reconcile: graph 1837, vectors 1858, in both 1837, C=1, B=0. The sentinel says sent=1858 and the vector store holds 1858, so THE SWEEP LANDED FULLY and the predicted partial-landing cause is refuted; the 21 are vectors whose Person the graph no longer has, which reads as the graph shrinking AFTER the upsert. CANNOT-DETERMINE on the mechanism: hydrate-people.log is absent. usage_journal_producers: 2 of 5 required producers wrote, 290 rows, 279 ingesting from fda and 11 answering. **fda went 0 to 279 BECAUSE OF THE WIPE** (the hydrate sentinel went, so the sweep ran), and **cm044 went 553 to 0 also because of the wipe**, but NOT because the wiki was empty: its summary backfill launched 18:48:11Z, wrote 0 bytes, and twenty minutes later was still 0 bytes with no process alive and both agents exiting 0. So waiting longer would not have reached five. cm048 and cm041 were simply not in this artefact and ship in v1.0.83. 📌 Andy was NOT asked to walk: item 3's bar is 0 FAIL and this is 6. **DEFERRED, NOT CLOSED**, for the standing reason (#931): the fixes for these are not in any artefact yet. Closes when a v1.0.83 walk moves them. | none |
| v1.0.83 |  | not_walked | Aesop 2026-09-09. **TAGGED ONCE AT db5319542064b67e8b023b1d06d7d8111ba89e0c AND NEVER BUILT.** Cut run 34396580800 concluded failure at 2026-09-09T19:48:59Z. PREFLIGHT SUCCEEDED (eight gates green, walk closure green after the v1.0.82 row landed); the CUT job failed at step 8, `Build, sign, notarise, staple`, at `make check-manifest`: 17 PASS, 1 FAIL, 54 SKIP. The one RED row is no-operator-hostname: a personal box hostname, 1 hit, in the shipped hub payload's vendor-manifest.toml, which is NOT hub-app content: gui/Makefile:1249 copies CM051's own vendor/VENDOR_MANIFEST.toml into ostler-payload at cut time, and that file gained the string at vendor/VENDOR_MANIFEST.toml:356 (a prose clause in the CM048 row's unverifiable_ack_reason) in CM051 #1881, merge e54e5a3e, the cm048 re-vendor. Measured both directions: 0 hits at tag v1.0.82 (718 lines), 1 at v1.0.83 (774 lines); the 082 DMG on disk reads 0 in that file and 0 app-wide. The gate was RIGHT. Steps 10, 11 and 12 then failed as CONSEQUENCES (`found no dist/*.dmg`), not independently, so this is ONE defect and not four. **There is NO walks/v1.0.83.tsv and there should not be**: no artefact ever existed. Checked all three release repos before writing this: no v1.0.83 release exists in CM051, in `ostler-ai/ostler-installer` or in `ostler-ai/ostler-releases` (control: v1.0.82 present). ONE TAG PUSH; the tag is SPENT per directive item 4; no second push. The fix is a one-file prose reword of that clause (CM051, item 2(c), with a PR-time check so the next such string dies at review and not at a tag), and the next cut is v1.0.84 from a branch point that carries it. Why every PR check let it through: the operator-pii-scan's pattern set has no hostname pattern; only cut-manifests/permanent.yaml's no-operator-hostname row carries it, and that gate runs at cut time. | build failed before any artefact; no probe measured; the hostname string is a personal identifier in a public repo's shipped file |
| v1.0.85 | 2026-09-09 | deferred | Archie 2026-09-09. **CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED, WALKED ON WIPED STORES, AND IT MISSED CLEAN BY ONE ARM OF ONE PROBE.** Artefact OstlerInstaller-1.0.85.dmg, sha256 5170fcaccbcd509eab4c3c955178bf420acf34696a19bc4e0f5f9f689899029f, 71,386,405 bytes, tagged ONCE at e39f767f17262cc75ec4790f6c5c9025b5b5f54b at 21:10:13Z, cut run 34405387365 success with EVERY step green. Record walks/v1.0.85.tsv (CM051 #1898): **25 pass / 1 fail / 0 cannot-run / 0 broken, 26 of 26, verdict FAILED, qa_exit 1**, walked_at 2026-09-09T22:15:01Z, instrument_rev e39f767f, box_fp 95b30a89637eecbb. 🟢 **usage_journal_producers PASSES FOR THE FIRST TIME, 5 OF 5.** Computed independently of the probe, from the journal on the box, matching each producer the way the ROSTER says to match it (roster staged from this tag, blob eaea9027, verified identical on the box by sha256): cm044_wiki_compiler 549 rows, cm048_conversation_extract 116, cm041_identity_resolution 877, cm051_ostler_fda_ingest 308, oa_daemon_chat 10 by PURPOSE, oa_proactive_cycle 0 and DORMANT. 1860 rows parsed, 0 unparseable, control prefix that cannot exist 0. **MY FIRST READ SAID 4 OF 5 AND WAS WRONG**: purpose is nested at usage.purpose, my script read it top-level, got <none> for all 1854 rows and scored the one purpose-matched producer 0. Same wrong-nesting-level defect corrected by hand on the v1.0.82 walk. Caught by the histogram reading <none> for EVERY row, a uniform value being the signature of reading the wrong level rather than of an empty field; the script now warns on a uniform histogram. 🟢 **cannot_run 0 AND stores FRESH, BOTH ON A WARM BOX.** No teardown-to-cold: Step B ran the shipped uninstaller, took 5 ostler_ volumes to 0, counted undeclared entries and content-root files to 0, and only then wrote the provenance marker; the licence was saved and restored at 447 B. Cold-then-prime never made stores fresh, it only gave the wipe something to remove, and when that same completeness check failed earlier in the project it REFUSED with CANNOT-RUN and wrote no marker, which is what makes the field a measurement rather than a fixture. C3 removed the 1.0.82 app only after reading its plist as 1.0.82, placed 1.0.85, codesign rc 0. 🔴 **THE ONE FAIL IS assistant_answers_grounded, 1 OF 4 ARMS, AND IT IS A DAEMON QUESTION.** Questions 1, 2 and 4 GROUNDED, including pwg_preferences on a seed the API compiled and served (CM051 #1888 doing on the instrument exactly what it claimed) and the fact-carrying assertion, so fact_missing is gone. Question 3 got tool_found_nothing from pwg_topics AND THE STORE WAS NOT EMPTY. Archie measured at 22:21Z: 18 ConversationTopic and 24 TopicMention by SPARQL with controls firing (57520 triples, 1837 Person, a class that cannot exist 0), and GET /api/v1/topics served 200 with the same count, against controls that the same token serves /api/v1/preferences 200 and that NO token gets 401. Aesop measured at 22:26Z with the filter varied: unfiltered 25, q=loft 2 (the seed's own two slugs), q=contact 0, q=<question 3 verbatim> 0. The count moved 18 to 25 in six minutes because the shipped feed drivers were still processing. **BISECTED: the daemon tool forwards the model's free-text query as ?q= (ostler-assistant crates/zeroclaw-tools/src/pwg_topics.rs:102, introduced at 7a6c7e31, 2026-06-22) and topics_list filters on a substring of slug-or-label (vendored cm041 assistant_api ical-server.py:4048), so the whole question matches nothing.** Plus the model's tool choice: it reached for pwg_topics on a CONTACTS question, and the routing clause at 80bf9615 did not hold on that turn. The exact query string is NOT in the record and is not guessed: the probe's frame transcript is a mktemp file the probe removes. ⚠️ **THE CONVERSATION SEED REPORTED CANNOT-RUN AND THE CAUSE IS APPARATUS, NOT THE BOX.** Its pipeline COMPLETED, CLI_RC 0, nine steps, 2 topics written at L2, and cm048 wrote rows; only its own read-back failed. lib/conversation_seed.sh does K="-K $CONF" and passes $K UNQUOTED; the box login shell is zsh, which does not word-split an unquoted variable, so curl receives ONE argument and reads a path with a leading space. Measured three ways on the box: the seed's form under zsh errors, `-K "$CONF"` as one quoted argument returns 200, and the seed's form under /bin/sh returns 200. **The same $K is used by the forget block (:903-960), so the seed's conversation point and its two topics were NOT removed** and q=loft 2 proves they remain; the disk forgets used rm and did work. This is the zsh seam in the very file CM051 #1885 touched, and #1885's own test never exercised a real curl. Post-freeze, or under item 2(a) only if it moves a probe, which it does not. 📌 **ANDY IS NOT ASKED.** Item 3's bar is 0 FAIL and this is 1. **DEFERRED, NOT CLOSED**: the fix for the one red is a daemon change and is in no artefact yet. Closes when a later walk moves assistant_answers_grounded. 🗿 **AND A STANDING NOTE FOR WHOEVER TOUCHES THE TOPICS SURFACE NEXT.** The walk account is now a LIVE DATA SUBJECT, not a synthetic fixture: 92 files across 45 conversation directories under its content root, date buckets running 2014 to 2026, all written during this QA window by com.creativemachines.ostler.imessage-bundle, which was still in state=running when this row was written. The uninstaller wipes ~/.ostler and not the content root, and it should not. Every count in the record is fine; a probe that printed a topic LABEL, a subject or a filename would put real conversation content into a PUBLIC repo. | none |
| v1.0.84 |  | not_walked | Aesop 2026-09-09. **TAGGED ONCE AT ba3215d51be8a12a90c8d0f9ff672064b4632be1 AND NEVER BUILT.** Cut run 34401516801 concluded failure at 2026-09-09T20:42:28Z. PREFLIGHT SUCCEEDED; the CUT job failed at step 8, `Build, sign, notarise, staple`, at `make check-freshness`, eight minutes in, AFTER `make check-manifest` had PASSED (18 PASS, 0 FAIL, 54 SKIP; the v1.0.83 hostname reword is proven in the artefact gate). The cut-freshness preflight read fresh=14 held=8 exempt=4 stale/RED=1: vendor:cm041/contact_syncer lags live upstream HEAD by ONE delta commit, a0f3ca19 (CM041 #161, the upstream PII scrub), which is NOT in that tree's hold_ack_shas. That commit merged upstream on the afternoon of 2026-09-09, after the v1.0.82 cut ran green through this gate at 16:28Z, and v1.0.83 never reached this gate because it died one gate earlier. Steps 10, 11 and 12 then failed as CONSEQUENCES (`found no dist/*.dmg`). The vendored contact_syncer ALREADY carries the scrubbed content (its locator block digest matches #161, measured in CM051 #1884) and the pin cannot advance by regeneration because the tree does not reconstruct at the pin (1 of 17 hunks, measured in CM051 #1878); so the honest exit is the gate's own remedy, a hold_ack naming a0f3ca19 with shipping_bugfixes_grafted = true and the reason written, not a re-pin tonight. **There is NO walks/v1.0.84.tsv and there should not be**: no artefact ever existed. No v1.0.84 release exists in CM051, `ostler-ai/ostler-installer` or `ostler-ai/ostler-releases` (control: v1.0.82 present). ONE TAG PUSH; the tag is SPENT per directive item 4. Next cut v1.0.85 from a branch point that carries the hold_ack. | build failed before any artefact; no probe measured; the manifest gate passed, so the v1.0.83 defect is closed in the artefact gate |
| v1.0.86 |  | not_walked | Archie 2026-09-10. **TAGGED ONCE AT fb1e7843f34f415b24771e0ac4bcb2e747cf8b6c AND NEVER BUILT.** Cut run 34441097038 concluded failure at 2026-09-10T05:29:08Z, 2 minutes 19 seconds after the tag push. PREFLIGHT PASSED: all EIGHT local gates green before the tag (cut pin current, vendored BOM fresh, walk closure, rollforward pin, artefacts the build fetches exist, BOM rows are in the pin, cut record exists, installer version IS the cut version). The CUT job failed at step 8, `Build, sign, notarise, staple`, at `make check-pr-age` (scripts/verify_pr_age.sh, merge or close within 48h, rule effective 2026-08-06), on TWO rows: CM051 #1818 open 49h, a DRAFT titled by Andy \[LAUNCH ITEM, do not merge until walk clears\], with no exemption; and CM051 #1755 open 72h with an EXEMPTION EXPIRED. **TIME, NOT CODE, TURNED IT RED.** The same gate PASSED at 21:10Z the previous evening for v1.0.85, when #1818 was 41h old and #1755's exemption was live. Nothing in this cut caused it: install.sh and gui/Makefile are byte-identical to the v1.0.86 branch point 4e4c6541, and the only source change since v1.0.85 is the 0.4.78 six-site re-pin. A gate keyed on WALL CLOCK can flip between two cuts with an unchanged tree. **AND NO LOCAL GATE COULD HAVE SEEN IT**: check-pr-age runs at cut time only and is not among the eight, nor among the PR checks on #1900, which read 49 PASS 0 FAIL. That gap cost this version number and is the transferable finding. The gate also declares itself PARTIAL: the runner token cannot list the other six repos, so its answer covers CM051 alone. Steps 10, 11 and 12 then failed as CONSEQUENCES (no dist/*.dmg), so this is ONE defect and not four. **There is NO walks/v1.0.86.tsv and there should not be**: no artefact ever existed. Release absence checked in three repos AND THE CONTROL ONLY WORKS IN ONE, which is stated rather than glossed: v1.0.86 is absent from ostler-ai/ostler-installer where v1.0.85 DOES resolve, so that reading discriminates; in CM051 and ostler-ai/ostler-releases both v1.0.86 and the v1.0.85 control read `release not found`, so those two readings prove only that DMG releases do not live there and are NOT evidence about v1.0.86. ONE TAG PUSH; the tag is SPENT per directive item 4; no second push. The fix is a register edit under freeze 2(c), CM051 #1901, adding the two exemptions with reasons and a review date, and the next cut is v1.0.87 from a branch point that carries it. The daemon work this cut existed to deliver is UNAFFECTED: hub-v0.4.78 is published and verified on downloaded bytes, and the six pin sites are on main at 4e4c6541. | build failed before any artefact; no probe measured; the two PR-age rows are register state, not artefact defects |
| v1.0.87 | 2026-09-10 | deferred | Archie 2026-09-10. **CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED, WALKED ON WIPED STORES, AND THE PROBE THIS CUT EXISTED TO MOVE MOVED.** Artefact OstlerInstaller-1.0.87.dmg, sha256 b7e1c9b9c4695d53d5976df4d0ba1286d5349916c5bbf18383a26c77abf4c91e, 71,391,211 bytes, tagged ONCE at 80f0fe08a72ba1e553bdf2b40b86c482cb7ef843, cut run 34443054038 (workflow `cut`, asserted by NAME because the tag fired five workflows and a headBranch match alone can select the wrong one) concluded success at 06:09:04Z with 0 failed steps. Record walks/v1.0.87.tsv (CM051 #1903, merged 4a3fd6ab), byte-identical to the walk agent's file at sha 6c54f5c7: **26 pass / 0 fail / 0 cannot-run / 0 broken, 26 of 26, verdict FAILED, qa_exit 1**, walked_at 2026-09-10T07:35:07Z, instrument_rev 80f0fe08 clean, stores_provenance wiped-by-explicit-store-wipe(0 ostler_ volumes remain) so FRESH. Ledger row HR015 #891 at fb74f17c. 🟢 **assistant_answers_grounded PASSES, 4 OF 4**, against FAIL on v1.0.85 where arm 3 returned tool_found_nothing. PROVEN under item 9 and EARNED, because both preconditions were measured before the result was trusted: C4 read the INSTALLED daemon rather than the bundled one (fix literal 1, commit 1d83d4bf 1, outgoing a9af0595 0, control pwg_topics 6, bundle 0.4.78), so the box ran the new daemon and install.sh did not take its fallback download path; and C5 read the topics API at 200 with count 0 BEFORE the seed and 14 AFTER, two of them the seed's own subject, so the tool answered FROM DATA. Without C5 a zero-topic store would have produced tool_found_nothing again and read as FAIL, indistinguishable from the fix not working; run_box_walk.sh:241-245 says an empty store cannot be told from a broken one. 🔴 **THE VERDICT IS FAILED AND IT COMES FROM PHASE 2, NOT FROM ANY PROBE.** Manifest runtime proofs read 65 PASS, 3 FAIL, 2 SKIP, 2 CANNOT-RUN of 72. (1) and (2) people_stores_reconcile twice in ONE re-run: graph 1838, vectors 1838, wiki tile 1839, where phase 1 read all three at 1839 and PASSED. Re-read DIRECTLY and still 1838/1838/1839, so not transient: the compiled wiki tile lags one merge behind the live graph until the next compile. A STALE-TILE shape, and the first reading of it as a lost person was wrong. (3) the acceptance gate, BLOCKED at 5 pass / 2 fail / 1 needs-eyes / 0 could-not-run: **FAIL A4 Pairing signals consistent**, companion=false paired=false token=true read from /health, signals DISAGREE, which the gate's own source at :157-171 names as the shipped lying-UI bug shape; and **FAIL A6 Wiki compiler clean (fresh image)**, sparql-400=1 (stale image, pre-#219), parser-crashes=0, broken-links=133. The two CANNOT-RUNs are the grounded probe hitting its 180 s cap in the REPLAY, having passed 4 of 4 in phase 1. 🗿 **A7 IS A FOOTER, NOT A CAUSE, AND WAS NEARLY RECORDED AS ONE.** The acceptance-gate manifest row captured only the line `(A7 needs-eyes is the one check that still requires a human walk.)`. Measured in the gate: result() at :103-111 increments `manual` for MANUAL and `fail` ONLY for FAIL, separate counters; A7 at :205 is `result MANUAL`; the header at :27-28 states NEEDS-EYES (printed, NEVER hard-fails); :238-241 prints that sentence AFTER `fail > 0`. Had it stood it would have meant no walk could read CLEAN before a human walked, making directive item 3's bar UNSATISFIABLE. It is satisfiable. **AND A4 AND A6 HAVE BEEN STANDING UNREAD BEHIND THAT FOOTER FOR AT LEAST THREE RECORDS**: neither the v1.0.82 nor the v1.0.85 QA log carries an A4 or A6 line, and the phase-1 pairing probe passes because it compares the device and policy layers for internal consistency and never reads token_paired. APPARATUS ITEM: a manifest row captures only the LAST stdout line of a probe. 📌 **ANDY IS NOT ASKED ON THIS RECORD.** Item 3's bar is 0 FAIL and only TCC/GUI cannot-runs; the probes meet it at 26/0/0/0 and the record verdict does not. **DEFERRED, NOT CLOSED**: A4 belongs to the daemon's /health token_paired semantics and A6 to the wiki image pin and its 133 broken links, both morning work with named owners, and the stale tile closes when the compiler runs. | 3 finding(s): the stale people tile, acceptance A4, acceptance A6 |
| v1.0.88 | 2026-09-10 | deferred | Archie 2026-09-10. **CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED, WALKED ON WIPED STORES, AND THE CUT'S OWN JOB LANDED WHILE A DIFFERENT PROBE WENT RED.** Artefact OstlerInstaller-1.0.88.dmg, 71,391,414 bytes, sha256 a659e1029546c5e39bc33947df94ed7b5de986e86566f17c52272429dc82fae8, verified by THREE independent instruments with the digest agreeing four ways. Record walks/v1.0.88.tsv, landed by CM051 #1908 MERGED 411cdeff04e3180cda0bd873a6a8b6deb5444908 at 12:35:00Z byte-identical (sha256 7a70825e), ledger row HR015 #896 MERGED d2e122f13278782ad16a5f84d5850e5dda3c1131 at 12:36:15Z, delivered row HR015 #895 at 0452086c. walked_at 2026-09-10T12:09:34Z, instrument_rev 2eca12e0 EQUAL to the tag, stores_provenance wiped-by-explicit-store-wipe(0 ostler_ volumes remain). Verdict FAILED and qa_exit 1, so the rules table above admits `closed` or `deferred` and REFUSES `not_walked`. **DEFERRED, NOT CLOSED.** PHASE 1 pass 25 / fail 1 / cannot_run 0 / broken 0, 26 of 26 measured. PHASE 2 70 PASS / 0 FAIL / 2 SKIP / 0 CANNOT-RUN of 72, manifest all rows satisfied. 🔴 **THIS CUT'S OWN TARGET MOVED AND EVERY v1.0.87 PHASE-2 RED IS GONE**: box-walk-acceptance-gate-v1013 PASS, both people_stores_reconcile rows PASS after the post-forget resync, both grounded replay rows PASS under the 600 s cap. A6 evidence string verbatim: `sparql-400=0 crashes=0 broken-link-lines=48 found=253(summed over 5 compiles) repaired=253 dead=0 repair-failed-files=0`, alongside A4 `companion=false paired=false devices=0 (unpaired, agree); token=true is the installer's admin token, expected`. Step E RESULT 7 pass / 0 fail / 1 needs-eyes / 0 could-not-run, PROBE GREEN. The discriminator that proves the RE-PINNED compiler ran rather than a cached one: all five compile summaries carry a degraded count, 39 + 88 + 42 + 42 + 42, summing to exactly the 253 found. **READ THAT STRING BEFORE THE VERDICT.** A green A6 with found=0 and a green A6 with found=133 are different results: the compiler logs a BROKEN LINK line for every link it finds and only then degrades it, verified INSIDE the shipped image, so the RAW count stays non-zero BY DESIGN while the graded count goes to zero. found=N is the live measure of the surviving CM044 source defect. AN A6 GREEN HERE IS FOUR THINGS: one part re-pin, one part rescope to the wiki logs, one part predicate (a repaired link is not a dead one), and ONE PART NOT FIXED AT ALL. Contained, not cured, and it belongs to CM044. 🔴 **THE ONE RED IS BLOCKING AND IT IS NON-DETERMINISTIC, WHICH IS WORSE THAN A FLAT FAILURE AND MUST NOT BE READ AS NEARLY CLEAN.** assistant_answers_grounded completed 1 of 4 questions without carrying the fact a pwg_ tool had returned, the seeded synthetic fact. The SAME probe passed 4 of 4 in the phase-2 replay minutes later on the SAME box against the SAME daemon, and read 4 of 4 on the v1.0.87 walk the previous night; the daemon did not move between those readings (0.4.78 at 1d83d4b, byte-identical in this DMG, both bundled copies measured). So it is NOT a v1.0.88 regression, and it is NOT a transient to wave through: **a BLOCKING probe that answers differently on two runs of the same binary means its GREEN proves nothing either**, which retroactively weakens every 4 of 4 this probe has ever returned, v1.0.87's included. CAUSE ESTABLISHED BY FILE AND LINE, NOT INFERRED: the tool result reaches the answering turn every time and the answer samples at TEMPERATURE 0.7, so this is the model's dice and not the plumbing. 🔴 AND THE RATE, MEASURED OVER FIVE SAMPLES ON THE SAME BOX AND THE SAME DAEMON, SAYS THE NON-DETERMINISM IS NOT SPREAD ACROSS THE SUITE, IT IS CONCENTRATED ON ONE QUESTION. The three fixed questions grounded 15 of 15. The SEEDED-PERSON question failed 4 of 5: phase 1 fact_missing, replay PASS, then three direct runs reading fact_missing, memory_only, memory_only. TWO DISTINCT FAILURE SHAPES ON ONE QUESTION: memory_only means the model never called the tool, fact_missing means it called it and dropped what came back. That REFINES the reading rather than confirming it: a uniform sampling wobble would have scattered across all four questions and it did not, so the seeded-person question is the subject and 25 of 26 was never the useful number. The single replay PASS is why this probe reads GREEN at all in phase 2, and on 4-of-5-fail odds a green from it is close to noise. THE RECORD ITSELF REFUSES TO CLASSIFY IT and says so in its own field, `regression_of ... CANNOT-CLASSIFY: history reaches v1.0.87, which records no probe names`, so the evidence that this is not a v1.0.88 regression is EXTERNAL to the file and is written here rather than back into it. It is fixed in NO repo, so closing this row would be true about nothing at all. **ANDY IS NOT ASKED ON THIS RECORD.** Item 3 asks him only at 0 FAIL with TCC or GUI cannot-runs only; this is 1 FAIL on a blocking probe. Under Andy decision A the record verdict counts and is never softened. The cannot_run column reads 0, which is the one thing this record says cleanly: nothing went unmeasured. 🔴 **CORRECTED 2026-09-10 BY ARCHIE, AND THE CAUSE THIS ROW GAVE IS WITHDRAWN.** This row attributed the v1.0.88 grounded FAIL to the daemon, with a rate of 4 of 5 on the seeded-person question against 15 of 15 on the three fixed ones, in two shapes (memory_only and fact_missing), caused by TEMPERATURE 0.7 sampling, the model's dice. **THAT ATTRIBUTION IS WRONG AND THE FAULT WAS THE PROBE'S PREDICATE.** carries() tested the seeded fact as a FIXED SUBSTRING of the expected phrase. THE SPECIMEN, from the v1.0.89 cold walk (run 3) and quoted by STRUCTURE with every value elided, because the format is the evidence and a name-shaped string has no place in a public repo: the daemon's tool-backed answer comes back as a list, `Job Title: <...>. Employer: <...>. Location: <...>. Last Interaction: <...>`. That reply scores NO on the old exact-phrase predicate and YES on the component predicate, **so the old predicate could not pass the answer format pwg_people produces AT ALL.** A reply carrying every component scored fact_missing by construction, which is why the rate looked like dice. The fix is CM051 #1916 (carries() reads every distinctive component ORDER-FREE, with the exact-phrase reading printed beside it as evidence). **WHAT THIS DOES AND DOES NOT OVERTURN**: the v1.0.88 record still reads FAILED and that is not being amended, because a record is what was measured. What is withdrawn is this row's CAUSE. It also means v1.0.89, whose whole purpose was an omission guard built for that cause, was cut against a verdict now attributed to the instrument, and that guard has never fired on any box. The retracted wording is left standing above rather than deleted, so a later reader sees what was claimed and what displaced it. | 1 finding: assistant_answers_grounded non-deterministic on the seeded fact (daemon drops a returned tool fact, unfixed anywhere); A6 contained not cured, found=253 repaired=253 dead=0 carried above |
| v1.0.89 | 2026-09-10 | deferred | Archie 2026-09-11, AMENDING MY OWN ROW WITH RUN 6. **SIX QA RUNS ON ONE ARTEFACT, FIVE INSTRUMENT SHAS, AND PHASE 1 IS NOW CLEAN FOR THE FIRST TIME IN THIS SEQUENCE.** Artefact OstlerInstaller-1.0.89.dmg, a453d415, tagged ONCE at 5c3ce6c1. **CANONICAL RECORD REMAINS RUN 5** (walks/v1.0.89.tsv, sha 56ebf834, CM051 #1918); run 6 is recorded here and kept at launch/walks_v1.0.89_run6_d3af86fb.tsv, sha256 59a2f8caad82ad5650295a5e3285f67f71e5c227425759a96c688ee6e8927b4d, instrument_rev d3af86fb. 🟢 **RUN 6, COLD BOX, PHASE 1 CLEAN: 26 pass / 0 fail / 0 cannot_run / 0 broken.** Install WALK PASS 21:17:34Z; the email batch finished ON ITS OWN at 02:25:52Z, 5 h 08 min after install, with NO agent touched and nothing switched off; QA 02:25:55Z to 02:55:58Z; first wiki wait CONVERGED at 477 s; grounded 4 of 4; usage cm044 549 rows. **EVERY RED THAT DOMINATED THIS PROJECT FOR THREE DAYS IS GONE FROM PHASE 1.** 🔴 **THE VERDICT IS STILL FAILED, ON ONE PHASE-2 ROW, AND IT IS THE FIRST BLOCKING RED OF THE WHOLE SEQUENCE NOT ATTRIBUTABLE TO AN INSTRUMENT DEFECT.** probe-no-unexpected-egress, BLOCKING per scripts/walk_promote_scope.tsv:60: "1 connection(s) attributable to Ostler reached a destination outside the LOCAL-NETWORK boundary that the declared ledger does NOT name, and that the live DERP map does not explain." THE SAME PROBE PASSED THREE OTHER TIMES IN ABOUT SIX MINUTES: phase 1 (10 of 27 sockets), the permanent phase-2 row at log line 651, and a re-run at 02:57:43Z (11 of 32), that re-run being a measurement to NAME the connection and not a reroll; the verdict stands. So a short-lived connection, alive during one of four runs. **THE HOLDER AND THE DESTINATION WERE NOT RECORDED, AND THAT IS THE WORST INSTRUMENT GAP OF THE SIX BECAUSE IT IS A DIFFERENT KIND**: verify_cut_manifest.py:1251 keeps only the first 200 characters of a failing probe's verdict line, and the crossing was printed above it. The other five gaps produced WRONG verdicts, which can always be re-measured; this one produced a verdict that may well be RIGHT and destroyed the evidence needed to act on it. A connection that lasted minutes cannot be re-measured. Post-freeze, ranked first: phase 2 must keep a failing probe's FULL stdout. **NOT REPRODUCED IN 20 MINUTES OF SAMPLING**, and that sentence is exact: not "did not happen", not "benign". The sampler's control is PROVEN, a planted 30 s crossing from a non-Ostler process logged within 2 s with holder, parent and remote, so the non-reproduction is a measurement and not a dead instrument. **ONE OSTLER-LINEAGE CROSSING WAS LOGGED AND IT IS NOT THE FAILING SHAPE**, verbatim: `03:10:28Z limactl pid=568 ppid=1 parent=/sbin/launchd remote=103.102.166.224:443`, bracketed by a wiki recompile whose compile finished 03:11:03Z. That address is Wikimedia's text front end and the ledger's wikidata rows (egress_hosts.tsv:82-83) resolve on the box to exactly it, so the probe credits it as DECLARED. **WHAT IT ESTABLISHES IS A HYPOTHESIS AND NOT AN ATTRIBUTION**: the compile container reaches the public internet during compiles, and the CM044 compiler fetches en.wikipedia.org BY CODE (compiler/wiki_enricher.py:23, music_enricher.py:24, called from pages/org_pages.py:681-701 for the customer's organisations, keep-alive httpx.Client, 29 GETs logged at 02:27:37-02:27:40Z), a host THE LEDGER DOES NOT NAME, which passes only by sharing an address with wikidata, while upload.wikimedia.org resolves on the box to 103.102.166.240, an address NO ledger row covers. **AGAINST THAT HYPOTHESIS**, and recorded because it cuts against it: the box's logs carry no compile and no compiler http line inside the failing window (last compile finished 02:49:26Z, next 03:11:03Z), and the iMessage feed through pwg-convo was the only Ostler process seen working through those minutes. **THE WINDOW IS BOUNDED AND THE CAUSE IS NOT ATTRIBUTED.** The crossing began after log row 651's three samples and was alive during row 692's; the 38 intervening rows include the acceptance gate under which compiles ran, and the feed was processing across the same minutes. The probes in that gap were checked for four launch verbs (open, kickstart, docker run, launchctl start) and had none, **WHICH DOES NOT EXCLUDE THEM**: a probe that merely queries the daemon can cause THE DAEMON to reach out, and the finding names an Ostler-attributable holder, not a probe. Conntrack inside the VM is unavailable, so container-level attribution needs nsenter or per-container sampling: post-freeze. ⚠️ **A SEPARABLE FINDING THAT SURVIVES WHATEVER CAUSED TONIGHT'S CROSSING**: install.sh declares NO `networks:` section at all and the wiki-compiler service declares no `network_mode`, so the compile container runs on the compose default bridge with OUTBOUND ACCESS THROUGH THE VM's NAT. The 46-row ledger describes what we EXPECT; the bridge decides what is POSSIBLE. Post-freeze in its own right. 🟢 **ONE ZERO-DENOMINATOR WAS HANDLED CORRECTLY TODAY AND IT IS THIS ONE**: run 6's record names NO probes (failed_probe_names_recorded "0 of 0", the phase-2 failure being invisible to the record, itself an instrument gap), and verify_walk_record.sh run against it exits **1** with "the record names no probes, so the promote scope cannot be applied; REFUSING UNSCOPED". It refuses because it cannot scope rather than reading an empty name list as nothing-failed. ⚠️ **AND THE CANONICAL RECORD IS THEREFORE MORE FAVOURABLE THAN THE LATEST MEASUREMENT, WHICH IS STATED HERE RATHER THAN LEFT TO BE DISCOVERED**: the gate answers OK on run 5 (whose only red is ADVISORY) and REFUSES on run 6 (whose red is BLOCKING). Run 6 is the later, colder, better-instrumented walk. **NO PUBLISH IS AUTHORISED ON THIS ARTEFACT ON EITHER RECORD**, and that bar does not depend on which one is canonical. **stores_provenance READS carried-over-from-previous-install AND THE BOX WAS MEASURED COLD** (colima 0, lima 0, ~/.colima 0 B, volumes 0, free space recorded, 20:52-20:54Z): ttywalk's marker vocabulary has exactly two words and no word for a cold box, so it emitted the nearest one, which is wrong. The consequence is concrete: verify_walk_record.sh's failure path prints "debug the data, not the build", **WHICH IS MISDIRECTION ON THIS RECORD**. Sixth instance today of one class, and the post-freeze item is not merely a cold-box word: A VOCABULARY WITH NO WORD FOR WHAT IT OBSERVED SHOULD REFUSE, NOT CHOOSE THE NEAREST LIE. 📌 **ANDY IS NOT ASKED TO WALK AND WAS NOT ASKED.** Item 3 admits 0 FAIL with only TCC or GUI cannot-runs. Told that the record is FAILED on an unattributed BLOCKING probe, Andy's decision, relayed: "stand down and I'll test this version when I get the time later today", and "This appears to be a known issue, which we can fix in the next turn". **THE DISTINCTION WAS PUT TO HIM AND IS RECORDED**: the 200-character truncation IS the known one-PR gap; THE CROSSING IS NOT A KNOWN ISSUE. He is testing the version himself knowing that. | 1 finding: an unattributed Ostler-attributable connection crossed the declared LOCAL-NETWORK boundary once in four probe runs, holder and destination destroyed by a 200-char truncation, not reproduced in 20 min of controlled sampling; plus three post-freeze instrument items (full stdout on a failing probe, a cold-box marker word or a refusal, per-container egress attribution) and one separable product finding (the compile container has unrestricted egress by configuration). Findings: v1089-D001, v1089-D002, v1089-D003, v1089-D004 |
| v1.0.81 | 2026-09-09 | deferred | Archie 2026-09-09. **CUT, SIGNED, NOTARISED, STAPLED, PUBLISHED, WALKED -- AND FAILED.** Artefact OstlerInstaller-1.0.81.dmg, sha256 3f3c13e29610883bfd0e0509598c96f784c4c18515b525b8d3f7f7bbb7e8e347, 71298427 bytes, tagged ONCE at f499a422914ae509acc84071d2e990dcd60f0a7d, cut run 34326753221 success. walks/v1.0.81.tsv (CM051 #1870, merged e208f9f0): 20 pass / 5 fail / 1 cannot-run, 26 of 26, 0 broken, verdict FAILED, qa_exit 1. The INSTALL completed clean: DONE status=ok, failed_steps=0, errors=0. artefact_sha256 is MEASURED, the first record since v1.0.68 that is actually bound to a build; v1.0.71, .78 and .79 all said asserted-unverifiable. That binding took a manual step and is not yet safe: ttywalk --from-dmg stages the DMG's CONTENTS and never the file, while post_walk_qa.sh:536 binds the record to the FILE on the box and verify_walk_record.sh:302-306 refuses anything not measured(...). The next walk hits the same wall. 🔴 **THE RESET DID NOT WIPE, BY DESIGN, AND IT SAID SO.** Corrected after I read ttywalk.sh rather than the report: a plain --reset is documented not to be a wipe (:533-553, decided on the v1.0.68 walk, "IT IS OPT-IN AND IT STAYS OPT-IN"), and when its loop matches nothing it PRINTS "NO SHIPPED UNINSTALLER FOUND. This reset did NOT uninstall", names all three searched paths, names ~/.ostler/bin/ostler-uninstall as the real one, and warns that a probe reading the stores is "measuring history, not this artefact" (:796-804). Adding the real path is refused BY NAME at :788 because the teardown inside it would wipe stores under investigation, and whether a walk wipes is the operator's decision. The harness told us exactly what it had not done and we walked without --wipe-stores anyway: that is on the operator, and the operator was us. Nothing was uninstalled, the Docker volumes survived, and stores_provenance is carried-over-from-previous-install. The store-reading probes measured HISTORY, which the walk gate reports of its own accord. **The two regression_of v1.0.79 rows (ingest_coverage, people_count_agreement) are regressions of a NUMBER on a box whose stores were never cleaned, and must not be bisected as this build.** 🟢 **THE 193 IS NOT TOMBSTONED MERGES, MEASURED NOT ARGUED.** oxigraph Person 1821; Persons WITH belongsToUser 1629, which equals BOTH the Qdrant people collection and the Doctor; Persons WITHOUT it 192, and all 192 carry displayNameProvisional with source=calendar_fda; merge or tombstone predicates in the ENTIRE graph: 2, not 192. The complement equalling the two other stores is the control that makes the FILTER NOT EXISTS reading trustworthy. So people_count_agreement compares a TOTAL against a USER-SCOPED subset, people_stores_reconcile's unsearchable set is the SAME 192, one cause for two reds, and the 192/193 drift is a live ingest population rather than a fixed shortfall. None of it is the 0.4.75 to 0.4.76 daemon bump. 🔴 **CORRECTION, mine.** I wrote that conversations was EMPTY and would stay so, off the Doctor's drain_completed_no_conversations. True at 08:34:56Z, wrong as a forecast. Measured on Qdrant with the same credential, two request shapes agreeing: points_count 0 (my read), 14 (Aesop's), 80 at 09:21:16Z. conversations is a LIVE ingest target, so ingest_coverage is a NOT-SETTLED-YET red and a harness ORDERING item (QA began about 61 s after install DONE, while ingest was still landing rows), not a permanent absence. A completed drain is not a closed store: later ticks land rows the drain status cannot see. Aesop proposed this and I argued against it. There is NO topics collection on the box at all, so pwg_topics finding nothing is CANNOT-RUN shaped, not empty shaped. **The one defect here that is nobody's environment: pwg_preferences returns nothing.** MEASURED since: the graph holds ZERO preference nodes (no pwg:LikePreference or pwg:DislikePreference, no preference predicate, across the default AND the named graph, both controls firing), and archie's interest_profile.json reports raw_rows 0, interests 0, suppressed_low_confidence 0, regenerated 09:27:42Z. Two independent instruments, the consumer's own accounting and the producer's store. So it is UPSTREAM of the generator and the min_confidence floor (CM051 #1872, held) is NOT the cause: with zero rows the floor is never consulted. The 830 Qdrant points were never that route's denominator and are rising (5652 by 09:35Z); preferences are written to the VECTOR store and never to the GRAPH, which is the same writer/reader split as the 192 persons. TNM has the producer hunt. converge_kill_is_recorded moved FAIL to PASS (CM051 #1857). 🔴 **THIS INSTALL SKIPPED THE VENDORED REFRESH, SO NO "DELIVERED" WORD IS AVAILABLE FOR ANY VENDORED COMPONENT ON THIS WALK.** Measured 2026-09-09T09:56:31Z on the box as archie: today's install ran 16:28:40 local and EIGHT of eleven venvs under ~/.ostler predate it (services/cm019 11:11:05, cm052 11:26:40, email-ingest 11:11:28, email-source 11:11:32, imessage-source 11:11:39, spoken-source 11:11:36, doctor 11:11:15, import-pipeline 11:10:43, all 2026-09-08), while only ~/.ostler/.venv 16:18:57, services/cm048 16:27:19, services/knowledge 16:27:27 and the store-auth .pth 16:17:17 are today's. Cause, found by TNM in this install's log: cm019_setup and its siblings skip on an existing interpreter and never refresh the vendored tree. Neither teardown reaches them -- --wipe-stores wipes STORES, and the #1828 config-only teardown removes config, security, assistant-config, imports and active_work. 🔵 **CORRECTION, MINE, SAME ROW.** I concluded from those venv dates that the staged CODE was stale for eight services and that no DELIVERED word was available for any of them. WRONG INSTRUMENT: a venv mtime measures the venv, not the code beside it. Measured in install.sh, which is the authority: cm019 at :19533 is guarded on the DESTINATION (`if [[ ! -x "$CM019_PY" ]]`) and that guard wraps the `cp -R` of the CODE as well as the venv and the pip install, so on a box with an interpreter the code is never re-copied (:19551 just prints already-set-up). Every other staging site is guarded on the SOURCE existing and copies every time: :18840 contact_syncer copies whenever the bundle is present, :19085 cm048 does `rm -rf` then a fresh `cp -R` on every install. TNM counts fourteen sites, thirteen unconditional, cm019 the lone exception. SO ONLY cm019's CODE IS STALE ON THIS BOX, not eight services, and a v1.0.82 vendored fix DOES reach contact_syncer, cm048 and the rest. The eight old venvs are old interpreters and dependencies, which matters only if a requirements change is needed and pip was skipped: a separate, smaller question. The right post-install check is a digest or mtime INSIDE ~/.ostler/services/<name>/ against the DMG bundle, because that shares a surface with the question; the venv mtime does not, and the install log never does. **AND THE PREFERENCES CASE IS CLOSED AT THE ROOT BY THE SAME LOG: the cm019 ingest NEVER RAN on this install** (import_pipeline only installs and prints the ostler-import command; cm019_setup reported already-set-up in 0 s; no ingest-dir anywhere). Nothing wrote the graph because nothing ran: not the shim, not a 401, not the min_confidence floor. My earlier 401 reading measured that the SERVER enforces auth, which it does, and did not measure the client, which the shim authenticates correctly (TNM, source and runtime, eleven of eleven venvs carrying the .pth). The v1.0.82 fix is the seed fixture seeding synthetic preference data through the ingest path, per item 5. Andy was NOT asked to walk: item 3's bar is 0 FAIL with only TCC/GUI cannot-runs, and this is 5 FAIL. 🟢 **RESOLVED FORWARD: ANDY DECIDED WIPE, 2026-09-09T13:4xZ.** The reason these five reds could not be attributed is that the stores carried over, which ttywalk documents as the operator's decision (ttywalk.sh:533-553, "IT IS OPT-IN AND IT STAYS OPT-IN"). It was put to Andy as an open question with no default and he answered with one word: "Wipe". So the v1.0.82 walk runs --reset --wipe-stores after the install brings the VM up, and its record's stores_provenance must read FRESH; carried-over there means the wipe did not happen and the three store probes are CANNOT-RUN rather than results. That is what will make ingest_coverage, people_count_agreement and people_stores_reconcile attributable for the first time. The "543 absent person nodes" justification at ttywalk.sh:788 is superseded for v1.0.82 by that decision; it is code-comment-only so it is untouched under the freeze and is restated with a reachable reference or retired in the post-freeze ttywalk reconciliation PR. | none |
| v1.0.90 |  | not_walked | ORM 2026-09-13, autonomous -- Andy asleep, explicit instruction to land the daemon pin and cut v1.0.91 without asking him anything. **NOT A NEW FINDING, SURFACED RATHER THAN DEFAULTED AROUND.** v1.0.90 shipped (OstlerInstaller-1.0.90.dmg, sha256 35ea17e7a0720be4eede163a4809bdc872f901c83556ec5025e3a7bfc3848fc4, 71398127 bytes, tagged once at c066755f0dc9ce9be5d40e24cc4c58897d826573, cut run 34675719471 SUCCESS, per SHIPPING_LEDGER.yaml) and was never box-walked; no walks/v1.0.90.tsv exists, so `not_walked` is the only legal status here (a walk record's absence refuses `closed`, per this table's own legality rule). Recorded here because `bin/rollforward_gate.sh --require-walk-closure` correctly refused to let v1.0.91 tag past a silent gap -- the mechanism did its job. **WHY IT IS DEFENSIBLE TO CUT PAST IT RATHER THAN WALK IT FIRST**: v1.0.90's only payload change over the walked-then-fixed v1.0.89 lineage was a one-line install.sh fix (Homebrew `tarball/master` -> `tarball/main`, CM051 #1920) to an install-path abort, guarded by a wired regression test; v1.0.91 changes only the daemon and hub-app pin (three UI/wiki-proxy fixes in ostler-assistant hub-v0.4.80, see cuts/v1.0.91/MUST_CONTAIN.tsv) and carries the tarball/main fix forward byte-for-byte in install.sh, so nothing v1.0.91 ships depends on v1.0.90 ever having been walked, and no risk v1.0.90 exists to cover is being carried forward unverified. **ANDY IS NOT ASKED PER ITEM 3** (its 0-FAIL bar governs the thin walk on the NEW artefact, not this table), but this row and its reasoning are put in front of him directly rather than silently defaulted, and v1.0.91 still needs its own thin walk before anyone is asked to walk it at the console. | none |
| v1.0.91 |  | not_walked | ORM 2026-09-13, autonomous -- Andy asleep, explicit instruction to complete this cut without asking him anything. **TAGGED ONCE AT 8650800c887f6b1bca28b64caa3f7bb01d17f2f8 AND NEVER BUILT.** cut run 34708384316's preflight passed and the build step died at check-pr-age: `make: *** [check-pr-age] Error 3`, VERDICT RED, PARTIAL -- 6 of 7 repo(s) NOT CHECKED and NOT DECLARED (andygmassey/CM044-PWG-Personal-Wiki, CM041-People-Graph, CM059-Ostler-Editor, CM031-PWG-Companion, HR015-Gaming-PC, and ostler-ai/ostler-assistant). **THE GATE WAS RIGHT, NOT REGRESSED.** CM051 #1930, merged the same day before this tag, changed an unreachable-and-undeclared repo from a silent GREEN, PARTIAL pass to a hard failure -- correctly, per its own measured finding that the six repos CI could not see carried 13 overdue PRs between them while the one it could reach reported clean. v1.0.90's own tag push (run 34675719471, five hours earlier the same day) hit the identical six-of-seven gap and read GREEN, PARTIAL under the pre-#1930 gate, so the underlying access gap is not new here, only the exit code is. **THE CREDENTIAL BEHIND IT HAS LAPSED**: CI's OSTLER_GH_TOKEN_ANDYGMASSEY does not cover pull-request read on these six repos, though a local `gh auth token --user andygmassey` reaches all six from a Mac -- a scope gap, not a networking or org-boundary fault, and not fixed by this row. CM051 #1942 declares the six by name via PR_AGE_ALLOW_PARTIAL (the escape hatch #1930 itself built for exactly this case) so v1.0.92 can carry v1.0.91's payload forward past it; the credential's actual scope is a separate, still-open item. **NOT WALKED because NOTHING WAS BUILT**: no dist/*.dmg, no artefact, nothing to walk, which is why `not_walked` here means something different from the v1.0.90 row above it -- that one shipped and sat unverified, this one never became an artefact at all. Spent, superseded by v1.0.92, and this is the fourth time this repo's own history shows a version number spent this way (v1.0.80, v1.0.83, v1.0.84, v1.0.86 each tagged once, built nothing, superseded by the next number carrying the same payload plus the one fix that unblocked it). | none |
| v1.0.92 |  | not_walked | ORM 2026-09-13, autonomous -- Andy asleep, explicit instruction to get past cut-machinery blockers without asking him anything. **TAGGED ONCE AT b6b667437bdae66937efd91cc5ddb78890252e2f AND NEVER BUILT.** cut run 34709712417 failed at check-manifest, a prerequisite of the build step, so no DMG exists to walk. Two operator-PII rows, no-operator-email and no-operator-wiki-name, both returned CANNOT-RUN: 660 files in the built OstlerInstaller.app carried a suffix on neither _TEXT_EXTS nor _COMPILED_EXTS (.icns, .car, .svg, .example and a nested .zip among them), so the scan had never opened them and correctly refused to call hits=0 a finding about the artefact. **THE SUMMARY READ 16 PASS 0 FAIL: nothing was wrong with the build, the gate simply could not reach a verdict** -- a zero denominator read as nothing-to-report rather than success, which is the correct refusal, not a regression. CM051 #1944 scans an unknown suffix with strings(1) and expands archives (strings cannot see through deflate), mutation-proven with a matched negative control; the refusal survives where it is real, an archive that will not expand still poisons the row. **NOT WALKED because NOTHING WAS BUILT**, same shape as the v1.0.91 row above it. Spent, superseded by v1.0.93 -- the fifth time this repo's history shows a version number spent this way (v1.0.80, v1.0.83, v1.0.84, v1.0.86, v1.0.91), each tagged once, built nothing, superseded by the next number carrying the same payload plus the one fix that unblocked it. | none |
| v1.0.93 |  | not_walked | ORM 2026-09-13, autonomous -- Andy asleep, explicit instruction to get past cut-machinery blockers without asking him anything. **TAGGED ONCE AT 766f12dd0e41ac99b75072b82cc82ee6538f9e05 AND NEVER BUILT.** cut run 34711604985 failed at check-manifest again, much closer this time: CM051 #1944 took the operator-PII scan's unread-file count on the built app from 660 down to 1. The survivor was python/lib/tcl9.0/cookiejar0.2/public_suffix_list.dat.gz -- a bare .gz is a single compressed stream with no container, so both zipfile and tarfile refused it, and that one file alone held all four operator-PII rows (no-operator-ip, no-operator-hostname, no-operator-wiki-name, no-operator-email) at CANNOT-RUN, 16 PASS 0 FAIL 4 CANNOT-RUN. **A SEPARATE CONCERN WAS RAISED AND RULED OUT ON THIS SAME RUN**: an 'ERROR: seeding ... disturbed the nested Ostler.app seal' line appeared in the log, prefixed with a bare '#'. Read in full context, it is quoted historical text inside the build script's own comment block (illustrating a v1.0.49 failure from 2026-08-28), not a live result; the actual check for this run, immediately after that block, read `[OK] nested Ostler.app: codesign rc=0, file added=0, file modified=0` -- a clean pass, confirmed before tagging v1.0.94 rather than assumed. CM051 #1946 decompresses single-stream .gz/.bz2/.xz before scanning. **NOT WALKED because NOTHING WAS BUILT**, same shape as the rows above it. Spent, superseded by v1.0.94 -- the sixth instance of this repo's spend-rather-than-rerun pattern (v1.0.80, v1.0.83, v1.0.84, v1.0.86, v1.0.91, v1.0.92). | none |
| v1.0.94 |  | not_walked | ORM 2026-09-13, autonomous -- Andy asleep, explicit instruction to get past cut-machinery blockers without asking him anything. **TAGGED ONCE AT 62db9cc8583c7e47e7f0ea1254ea6cb502288a45 AND NEVER BUILT.** cut run 34713065571's check-manifest and check-pr-age were BOTH fully clean (20 PASS 0 FAIL 0 CANNOT-RUN; PR-age VERDICT: GREEN, PARTIAL (DECLARED) naming six repos, exactly as CM051 #1942 intended). The NEW blocker was check-freshness: wiki:wiki-compiler and wiki:wiki-site both STALE:+12 against CM044 main. **THIS ONE WAS A REAL PRODUCT GAP, NOT A MACHINERY FALSE POSITIVE, AND WAS TREATED AS ONE**: the twelve unshipped commits included CM044 #275, the decimal-hour ages fix Andy had asked for repeatedly, plus per-source status icons, a duplicate mkdocs nav entry, and the People/timeline + People/index dated-list treatment. A hold_ack was available and was NOT taken: it would have asserted shipping_bugfixes_grafted = true while shipping none of the twelve, which is the exact failure Andy named as the one he most wants stopped. **RESOLVED BY REBUILD, NOT ACKNOWLEDGEMENT**: CM044 tagged v0.1.32 = 67fe4054, its own release-images.yml CI rebuilt both images (run 34713998954, SUCCESS -- the manual local build path was tried first and correctly denied, ghcr.io/creativemachines-ai requires the CM_AI_GHCR_PAT classic PAT, a CM044-only Actions secret this operator does not hold), and CM051 #1948 re-pinned both digests, verified BY CONTENT rather than by either job's claim. **NOT WALKED because NOTHING WAS BUILT**, same shape as the rows above it. Spent, superseded by v1.0.95 -- the seventh instance of this repo's spend-rather-than-rerun pattern (v1.0.80, v1.0.83, v1.0.84, v1.0.86, v1.0.91, v1.0.92, v1.0.93). | none |
| v1.0.95 | 2026-09-13 | deferred | Archie 2026-09-13. **IT SHIPPED AND IT WAS WALKED, AND THE WALK SAID FAILED.** Both halves verified rather than assumed: walks/v1.0.95.tsv exists, and the v1.0.95 tag on the releases repo carries 3 assets while v1.0.94 returns 404, so the published-assets check discriminates and is not answering yes to everything. Record: walked_at 2026-09-13T00:45:49Z, instrument_rev 236d667d EQUAL to the tag, verdict FAILED, qa_exit 1. **THE RECORD IS NOT GAPPY AND MY FIRST READING OF IT WAS WRONG.** `failed_probe_names_recorded 1 of 3` reads like two failures went unnamed. It is not: the 3 is the count of probes that CARRIED A PHASE 1 VERDICT FORWARD, and they held three DIFFERENT verdicts. assistant_answers_grounded FAIL, recorded by name in `failed_probe`. usage_journal_producers CANNOT-RUN, recorded by name in `not_measured_probe`, which is the correct field because CANNOT-RUN is not FAIL. ingest_coverage PASS. So exactly one failure existed and exactly one was named, and the record honoured the three-outcome split it is supposed to honour. **THE DEFECT IS THE CARRY-FORWARD ITSELF, NOT THE NAMING.** Verbatim from the post-walk log: "26 row(s), 3 marked seed-fixture; 6 row(s) took the phase 1 verdict (fixture present then, forgotten since) and were not re-run", each of the three probes appearing twice. Those three took a verdict measured while the seed fixture was present and kept it after the forgets, so their v1.0.95 result describes a box state that no longer existed when the record was written. A carried verdict is not a measurement of the artefact under test, and one of the three carried a PASS, which is the direction that hides rather than shouts. **WHAT THE FAILURE ACTUALLY WAS**, phase 1 at 2026-09-12T23:46:14Z with the seed fixture present and the negative control run first: "1 of 4 questions COMPLETED without reaching the customer's own data: [memory_only]". memory_only means the model never called the tool, which is a different shape from the fact_missing seen on v1.0.88 and is the shape a carried verdict cannot tell you is still true. usage_journal_producers could not run because cm044_wiki_compiler wrote nothing into the costs journal and the wait for its summary backfill did not converge, blocked before its first line. **stores_provenance reads unknown-no-reset-step**, so the box's starting state is unestablished for this walk too. 🔴 **AND THE ONE GREEN IN THAT LOG IS THE GAP WE SPENT 2026-09-13 FINDING.** Its store-surface probe returned "VERDICT: PASS, none of the 7 store/UI surfaces served an uncredentialled request", reading ports 6333 7878 8044 3000 6379 with 8000 confirmed open in the same run so the probe was live and not blind. **8089 IS NOT IN THAT LIST.** The Doctor was never asked. On that very box, 34 Doctor routes required no credential and POST /api/v1/auth/chat-token minted a device bearer to any caller, which CM051 #1962 has since partly closed. A probe that measures seven surfaces and calls the result a verdict about the install is reporting on its own population, not the product's. That is why this row is DEFERRED and not CLOSED: the walk's own FAIL is unfixed, and the probe population is wrong in a way no number in this record shows. **ANDY IS NOT ASKED ON THIS RECORD**: item 3 asks him only at 0 FAIL with TCC or GUI cannot-runs only, and this is a blocking FAIL. | none |
| v1.0.97 | 2026-09-13 | not_walked | Archie 2026-09-13. **TAGGED ONCE AT 258c7944192a3f305ae0c3a0248b751e9256068d AND NEVER BUILT.** cut run 34763644564: preflight SUCCESS, cut FAILURE, dry-run SKIPPED. No DMG, nothing signed, notarised or published, no release created, so `not_walked` is the only legal status here (this table's own rule: a walk record's absence refuses `closed`). Run selected by workflowName = cut and asserted before reading, because a tag fires five workflows. **THE PREFLIGHT PASSED, WHICH IS THE DIFFERENCE FROM v1.0.96.** The cut paperwork was right this time. The cut JOB died in the build step: `make: *** [check-pr-age] Error 1`, on one expired PR-age exemption, CM051 #1818 open 130h. Everything after it reads "found no dist/*.dmg. It has NOT verified delivery, it failed to look. DO NOT SHIP", which is three gates refusing to report a pass on an absent artefact and is correct. #1818's exemption was RE-DECIDED rather than rolled (CM051 #1966): its original ground was the launch freeze and said in its own text "retire this row when the freeze lifts"; the freeze HAS lifted, so renewing on that ground would have been a lie. The ground that survives is Andy's, in the PR's own title, do not merge until the walk clears, and the retirement condition is now a CONDITION rather than a date. 🔴 **THE CAUSE WAS THE TOOL, NOT THE EXEMPTION, AND THAT IS THE FINDING.** scripts/new_cut.sh printed "ALL GREEN. Safe to tag" after running only the gates cut.yml checks in its PREFLIGHT job. `make ship` has TWELVE prerequisites of its own and check-pr-age is one: the preparation gates and the ship gates are different populations, and the tool claimed a scope it did not have. The operator ran every gate offered and tagged on green. BOTH v1.0.96 AND v1.0.97 ARE DOWNSTREAM OF THAT SENTENCE. CM051 #1967 removed it: the green path now names its own scope, says it is NOT safe to tag, and prints the workflow_dispatch command. The file's own comments already recorded this class three times ("a checklist that omits a hard gate and then declares itself complete is worse than no checklist, because it converts 'I forgot' into 'the tool told me I was done'"), and the earlier fixes each ADDED the missing gate and the list drifted again within a day, which is why this one states the scope instead. **A DRY RUN AFTERWARDS COST NOTHING AND FOUND EVERYTHING.** workflow_dispatch run 34764181672 against main: preflight success, dry-run failure, cut SKIPPED, "NOTHING SHIPPABLE WAS MANUFACTURED", no tag spent. Exactly one red, check-ship-version, refusing because v1.0.97 was already tagged, which is correct and is the spent tag. check-pr-age read GREEN, PARTIAL (DECLARED) and check-freshness read stale/RED=0. That instrument existed before today and the v1.0.90 row names it. ⚠️ **NOTE FOR WHOEVER MAINTAINS THIS GATE**: v1.0.96 was also tagged and also built nothing, and it is NOT named by walk closure, because this gate's population is cuts/<version>/ DIRECTORIES and v1.0.96 never got one. A cut with no paperwork is invisible here. It is filed in SHIPPING_LEDGER.yaml dmg_cuts as TAGGED_PREFLIGHT_REFUSED_NOTHING_BUILT, so it is recorded, but not by this instrument. Spent, superseded by v1.0.98 carrying the identical payload. | none |
| v1.0.98 | 2026-09-16 | not_walked | Aesop 2026-09-16. **TAGGED ONCE AND ACTUALLY BUILT**, which is what separates it from v1.0.91 through v1.0.94: cut run 34768023727, conclusion SUCCESS, 2026-09-13T16:15:19Z, and a DMG reached Andy. **`not_walked` IS NEVERTHELESS THE ONLY LEGAL STATUS**, and the reason matters: walks/v1.0.98.tsv DOES NOT EXIST (control: walks/v1.0.95.tsv is 87 lines, so the check discriminates). The thin-walk instrument never produced a record for this artefact, and this table's own rule refuses `closed` without one. **ANDY WALKED IT HIMSELF AT THE CONSOLE AND IT FAILED.** He reported five defects in his own words: the recovery-key modal flashes up instantaneously at the end; the wiki via a browser demands authentication details he does not have; the wiki inside the Ostler app is broken; the front page shows less than it did before; and the People count at 3,750 is far lower than expected. He then typed `ostler-unlock` and got `command not found`. So this cut was verified by a person and the person found it broken: the absence of a machine record is a gap in the INSTRUMENT, not evidence that nothing was learned. **EVERY ONE OF THOSE FIVE HAS A FIX IN FLIGHT OR MERGED FOR v1.0.99**, which is what that cut is: Doctor 500 (#1971, merged), the vocabulary mismatches behind the thin front page (#1972, merged), forget-deletion (#1973, merged), the wiki password prompt (#1980), the recovery key unreachable on a failed install (#1982), and `ostler-unlock` on PATH (#1969, merged). 🔴 **A SECOND GAP FOUND WHILE WRITING THIS ROW, AND IT IS NOT THIS TABLE'S**: SHIPPING_LEDGER.yaml has NO dmg_cuts row for v1.0.98 either. Its newest is v1.0.95, so a cut that tagged, built, published and was walked by the owner is absent from the register whose entire job is recording what shipped. Two independent registers both missed the same cut, which is why walk closure refused and why that refusal was right. | none |
| v1.0.99 | 2026-09-17 | not_walked | Aesop 2026-09-17. **TAGGED ONCE AT d5d401358c9b4527bc86577df9622474cf83a8a9 AND NEVER BUILT.** cut run 35187827169: preflight SUCCESS, cut FAILURE, dry-run SKIPPED. No DMG, nothing published, no release created, so `not_walked` is the only legal status (walks/v1.0.99.tsv DOES NOT EXIST; control: walks/v1.0.95.tsv is 87 lines and walks/ holds 20 records, so the check can find one when there is one). **THIS ONE GOT FURTHER THAN ANY FAILED CUT BEFORE IT.** Both apps built, both signed, round 1 AND round 2 submitted, and Apple ACCEPTED round 2 (submission 8f7a4ff0-ba1d-4fc0-8f42-1947ccf40cf0). It died at `make: *** [staple-apps] Error 1` on the post-staple validate. Steps 10, 11 and 12 then read "found no dist/*.dmg" and refused, which is three gates correctly declining to report a pass on an absent artefact: ONE defect, not four. 🔴 **THE CUT MACHINERY WAS THE BUG, WHICH IS WHY ITEM 4 APPLIES AND THIS VERSION IS SPENT.** gui/Makefile enumerated nested .app bundles into a variable and iterated it UNQUOTED, in both the Round 1 staple and the verify after it. That splits on IFS, which includes the SPACE. CM051 #1970 added "Recover Ostler.app", the first bundle in this product whose name contains one, so the single path Contents/Resources/Recover Ostler.app became the two paths that do not exist, Contents/Resources/Recover and Ostler.app. Each was reported "[skip] (no ticket issued for this CDHash)", which is INDISTINGUISHABLE from a bundle Apple legitimately issued no ticket for, and then "[FAIL] ticket missing after staple". The bundle that actually needed the ticket was never stapled and never named in the log. 🟢 **v1.0.98 IS THE CONTROL AND THE ARITHMETIC IS THE MEASUREMENT.** Same code, same runner, same step, differing only in the presence of a space-named bundle: v1.0.98 reported "4 bundle(s) required a ticket, 1 deliberately excluded" and every one passed; v1.0.99 reported 6 with the same 1 excluded. One real bundle added, counted as two. A count wrong by exactly the number of spaces in a name is a fingerprint, and it identified the defect before anyone read the loop. ⚠️ **THE DRY RUN WAS GREEN AND COULD NOT HAVE CAUGHT IT.** workflow_dispatch run 35186525129 immediately before the tag: preflight success, dry-run success, cut skipped. The dry run does not build, sign or staple, so this failure mode is outside its population by construction. That is the SAME lesson the v1.0.97 row above records about new_cut.sh claiming a scope it did not have, one round later and on the other side of the tag: the preparation gates and the ship gates are different populations. 🔴 **A SECOND LANDMINE WAS THEN FOUND BEFORE v1.0.100 WAS SPENT, NOT BY IT.** tests/test_installer_version_consistency.sh derived the build number with `[0-9]{1,2}` for the patch component. Measured with the two-digit neighbours as the positive control: 1.0.98 -> 9800, 1.0.99 -> 9900, 1.0.100 -> rc=2 CANNOT-RUN. check-version is a prerequisite of `package`, so v1.0.100 would have died on paperwork before the build, the way v1.0.96 did. Fixed in CM051 #2092. **BOTH WERE FOUND BY ONE QUESTION**: what is FIRST-OF-ITS-KIND about the next artefact. A space in a bundle name, and a third digit in a patch version. A value no code has ever seen is where the cut breaks. **FIXES**: CM051 #2091 (merged, the staple loops) and #2092 (merged, the derivation). Spent, superseded by v1.0.100. | none |
| v1.0.100 | 2026-09-17 | deferred | Aesop 2026-09-19. **IT SHIPPED, IT WAS WALKED, AND THE WALK SAID FAILED.** walks/v1.0.100.tsv exists and is 105 lines, so `not_walked` is a false statement contradicted by a record. CONTROL, because a record's mere presence proves nothing about the check: the four rows directly above this one are all `not_walked`, and for each of those walks/v1.0.NN.tsv does not exist, so this table can tell the two apart. verdict FAILED, qa_exit 1, walked_at 2026-09-17T14:10:01Z, box f0a5e4fd0536754c, walk_kind thin, and the record states in its own words that a thin ssh walk does NOT authorise repointing the customer download. COUNTS: pass 19, fail 6, cannot_run 3, broken 1, measured 28 of 29. SEVEN failed_probe names are recorded while fail reads 6, and that is NOT an inconsistency: counts_scope says the four counts are box_walk_probes_only(phase1) while failed_probe covers all phases, so the seventh is a phase-2 failure sitting outside the phase-1 denominator. THE SEVEN: acceptance_gate_v1013, assistant_answers_grounded, install_manifest_complete, no_store_port_is_tcp_reachable, no_unexpected_egress, ostler_unlock_reachable_by_name, people_count_agreement. EVERY ONE IS A REGRESSION rather than a newly added probe: the record carries a regression_of row for all seven. FOUR WERE NOT MEASURED AT ALL: assistant_grounds_the_opening_turn (which is also the single broken_probe), doctor_page_renders_for_a_customer, ingest_coverage, usage_journal_producers. A not-measured probe is not a pass and none of the four is counted as one. THE ARTEFACT HASH IS NOT KNOWN: artefact_sha256 is unavailable and its source is asserted-unverifiable(no OstlerInstaller-1.0.100.dmg on the box), so this walk names a VERSION and cannot name the BUILD; stores_provenance is carried-over-from-previous-install, so the graph it measured was NOT created by this artefact. WHY `deferred` AND NOT `closed`: closed requires the findings set to equal the `### v1.0.100-Dxxx` sections in this file, and there are none. The seven failures are being worked as CM051 board rows rather than as sections here, and a row must not claim closure for work that is still open. | none |
| v1.0.101 | 2026-09-24 | deferred | Aesop LANDER 2026-09-24. **BUILT AS A workflow_dispatch CANDIDATE, NEVER TAGGED, AND WALKED TWICE.** walks/v1.0.101.tsv (committed with the v1.0.102 cut record): verdict FAILED, qa_exit 1, walk_kind thin, walked_at 2026-09-24T07:00:23Z, pass 28 fail 0 cannot_run 1 broken 0, measured 29 of 29. The single failed_probe is acceptance_gate_v1013 (a phase-2 gate outside the phase-1 counts, recorded as regression_of v1.0.95); four probes not measured (assistant_grounds_the_opening_turn, doctor_page_renders_for_a_customer, ostler_unlock_reachable_by_name, pair_state_agreement), none counted as a pass. artefact_sha256 is unavailable in the record (the harness could not read it back), so the record names a VERSION; the walked DMG was sha256 2e845946... from cut run 35897377187, artefact 287. Andy then walked the same DMG at the console on the Mini 16: DONE status=ok failed_steps=0 errors=0, 45 of 45 steps. **THE WALK'S REAL FINDINGS WERE ON THE DATA SOURCES, NOT THE PROBES**, measured on the box afterwards and filed as CM051 board rows: #2347 email emitted 0 in 12 ticks (checkpoint outside ~/.ostler), #2348 1,298 photo events extracted and read by nothing, #2351 Notes and Mail never warmed, #2353 AI conversations posted to the wrong port and the WhatsApp feed silent about why, #2355 five feeds on a fixed window, #2357 the wiki password in install.log; plus #2342 and #2344 from Andy's console walk. All but #2342 and #2344 are fixed on main and ride v1.0.102. **DEFERRED, NOT CLOSED**: the fixes are in no tagged artefact, and the findings are worked as board rows, not ### sections here, so a row must not claim closure. | none |
...
# EXIT 97, NOT 1. 97 is the runner vocabulary for "I could not look", as
# distinct from "the defect is present". Exiting 1 made this gate report as a
# MEASURED PRODUCT FAILURE on any box without a compiled wiki -- and this is
# the gate that closes CM051 #1619, so that misreading says the kinship defect
# is still present when nothing was examined at all.
# Measured 2026-09-07 against the wiped walk box:
#   before  ROLLFORWARD RED -- 1 measured failure(s), 0 CANNOT-RUN
#   after   ROLLFORWARD RED -- 0 measured failure(s), 1 CANNOT-RUN
# Both RED, so it never failed OPEN. It simply named the wrong cause.
# ONLY OFFENDER: 26 of the 28 gate bodies declare CANNOT-RUN and 25 of them
# already used the right code.
...
# THE READ CARRIES ITS CREDENTIAL, BECAUSE THE STORE NOW DEMANDS ONE.
# MEASURED 2026-09-06 on a walked box: OSTLER_STORE_AUTH_ENFORCE defaults to 1,
# so an unauthenticated SPARQL GET returns 401 with an HTML body and this limb
# reported "did not return a count (got '</html>')" -- honest, and a total loss
# of coverage that nobody saw because it failed SAFE. The same query with the
# box's own store-curl.conf returns 1835.
# The conf is the mechanism the box already ships for exactly this: 0600, in
# the operator's secrets dir, and already consumed by
# scripts/box_walk_probes/probes/people_seed_and_retrieval.sh.
# If it is ABSENT the read stays unauthenticated and the integer check below
# still turns a 401 into CANNOT RUN. That is deliberate: this limb must never
# convert "I could not read the store" into a pass.
STORE_CONF="${OSTLER_PROBE_STORE_CURL_CONF:-$HOME/.ostler/secrets/store-curl.conf}"
[ -r "$STORE_CONF" ] && OXI_AUTH="--config $STORE_CONF" || OXI_AUTH=""
# shellcheck disable=SC2086
ask() { curl -sS -m 60 -G $OXI_AUTH --data-urlencode "query=$1" -H 'Accept: text/csv' "$OXI/query" 2>/dev/null | tr -d '\r' | tail -1; }
...
### v1066-D011 -- the LaunchAgent prune promises a DATED archive and writes a single fixed slot per label

**OPEN. Low severity, and it is a comment-versus-code mismatch with a real
consequence, not a cosmetic one.**

`install.sh` prunes Ostler LaunchAgents whose program no longer exists. Its
own comment states the safety property it is relying on:
...
The directory is not dated, and neither is the file:

```
_PRUNE_QUARANTINE="${HOME}/.ostler/quarantine/launchagents"
mv "$_pl" "${_PRUNE_QUARANTINE}/${_pl_label}.plist"
```

One fixed slot per label. **A second prune of the same label silently
overwrites the first archive**, so the evidence that survives is only ever the
most recent prune of each agent, not the history the comment promises.

**MEASURED on the live v1.0.66 install, archie@.240, 2026-09-05:**

```
archived files                                   9
distinct labels                                  9
filenames carrying any date or timestamp         0
```

Nine files, nine labels, one slot each. Consistent with overwrite, and
inconsistent with the comment.

**WHY IT IS NOT WORSE THAN IT LOOKS.** The prune is correctly narrow: only
`com.ostler.*` and `com.creativemachines.ostler.*`, only when the program path
is absolute and does not exist, and an unreadable plist is skipped rather than
pruned because a CANNOT-RUN is not a licence to delete. Nothing here deletes a
customer's own agents. The defect is confined to the recoverability claim.

**WHY IT SURFACED AT ALL.** Five of the nine archived labels are ALSO live in
`~/Library/LaunchAgents` on that box. That is NOT a defect: the archives are
dated 02:48 from an earlier install run, and the later run recreated the
agents. Tracing that apparent contradiction is what exposed the naming.

**NOT THE SAME DEFECT AS v1066-D010.** D010 is an agent booting ITSELF out
before deleting its own plist, where the bootout kills the caller. This prune
runs inside the INSTALLER, booting out a different process, so the ordering
here is safe and the `mv` does execute.
...
⇒ I predicted: **a fresh account would not have them**, and named the test --
Andy's hand-walk on a NEW account (`archie2` on the Mini 16, confirmed by
hardware: Mac16,10, 16 GB, UUID E9589EC6), with the rule that if D003 and D004
do not appear there the archaeology diagnosis is confirmed, and if they DO
appear it is wrong.

🔴 **THE PREDICTION IS REFUTED. MEASURED ON archie2, 2026-09-05T11:28Z, ON A
GRAPH THIS INSTALL BUILT FROM SCRATCH MINUTES EARLIER:**

```
                                  archie (months of history)   archie2 (VIRGIN)
no_person_holds_two_contact_cards        54 over-merged              53
                                                                     113 cards collapsed
                                                                     worst node: 5 cards -> 1
people_stores_reconcile                  13 orphan vectors           38
```

⇒ **BOTH APPEARED ON A GRAPH WITH NO HISTORY.** These are LIVE DEFECTS. The
over-merge is produced by an ordinary install on fresh data, and a shipping
change CAN close it -- the opposite of what this section concluded.

⚠️ **53-VERSUS-54 ON TWO INDEPENDENT GRAPHS IS THE USEFUL PART.** Two accounts,
two container VMs, two sets of store volumes, one source of contacts, and the
counts land one apart. That is not drift; it is **deterministic from the source
data**, which makes it reproducible and debuggable. Start from the 53 on
archie2: they were made in the last hour and their inputs are still on the box.

⚠️ **WHAT I GOT WRONG, STATED AS A MECHANISM SO IT DOES NOT RECUR.** The
measurements behind the archaeology claim were all sound: no merge on the archie
graph had reached its delete step, no wholesale node delete exists in the
shipped tree, and the syncer mints an identifier under the holder it matched.
Those refute the TORN-MERGE mechanism. **They do not license the conclusion that
no live writer is responsible.** I turned "I could not find the mechanism" into
"the mechanism is not in the tree", and a human walk on a clean box refuted it
in one measurement. OS003 #217, which argued that conclusion at length, was
CLOSED UNMERGED rather than corrected after the fact.
...
### v1089-D001 -- a customer's calendar is silently unread when it arrives inside a platform export

**OPEN. PRODUCT.** The export scan stops at depth 3 (install.sh:9559-9560); the two standard exports
that carry calendars hold them at depth 5 and 7. Measured on a real box: 10 present, 0 visible. The
customer is told nothing was found. Detail and the refused workaround: `cuts/v1.0.89/DEFECTS_LEDGER.md`.
Ungated while open: the fix is a depth change plus a test that fails at the old depth, and it is not
freeze-eligible as item 2 is written.

---

### v1089-D002 -- an Ostler-owned process crossed the declared egress boundary, and was never identified

**OPEN. CAUSE UNKNOWN.** Run 6 phase 2: one connection outside the LOCAL-NETWORK boundary that the
ledger does not name. Three passes and one failure in about six minutes. Holder and destination lost
to v1089-D003; 20 minutes of sampling with a proven control did NOT REPRODUCE IT, which is not
evidence it did not happen. Ungated while open: no gate can be written for a crossing whose identity
was never recorded. Detail: `cuts/v1.0.89/DEFECTS_LEDGER.md`.

---

### v1089-D003 -- a failing probe's evidence is discarded, twice, so a real finding cannot be acted on

**OPEN. INSTRUMENT.** `scripts/verify_cut_manifest.py:1251` keeps 200 characters of a failing probe's
verdict line, and the walk record's `failed_probe_names_recorded` reads "0 of 0" because phase 2 names
no probe at all. Five earlier instrument gaps in this cut produced wrong verdicts, which re-measurement
corrects; this one destroyed the evidence for a verdict that may be right. `verify_walk_record.sh`
refused the record rather than reading an empty name list as nothing having failed, which is the one
zero denominator handled correctly. Detail: `cuts/v1.0.89/DEFECTS_LEDGER.md`.

---

### v1089-D004 -- the wiki compile container has unrestricted outbound access

**OPEN. CONFIGURATION.** The compile service declares no network mode and the install declares no
networks section, so the container takes the compose default bridge and reaches the public internet
through the VM's NAT. True whether or not it caused v1089-D002, and filed separately so it does not
close with it. Detail: `cuts/v1.0.89/DEFECTS_LEDGER.md`.
