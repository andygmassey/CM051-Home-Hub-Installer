...
| v1018-D012b | Verify chat's graph-grounding tool queries correct ontology + named graph (schema.ostler.ai/ontology#Person in urn:ostler:user/Andy, not schema:Person default) | [v1.0.18](v1.0.18/DEFECTS_LEDGER.md#v1018-d012b--verify-chats-graph-grounding-tool-queries-the-correct-ontology--named-graph) | **VERIFICATION PERFORMED 2026-09-16.** It found a defect, and a worse one than the row asked about. `_sparql_select` (ical-server.py) and `_sparql_query` (meeting_syncer/brief.py) queried the DEFAULT graph with no GRAPH clause, no FROM and no `--union-default-graph`; CM048 writes `urn:ostler:user/<id>`. Measured live: 37,625 triples invisible; `OutstandingTodo` 68 named / 0 default; `urn:ostler:warmth` 383 / 0; `urn:ostler:fromConversation` 1058 / 0. Consumer-side: `/api/v1/commitments` returned count 0 while `/api/v1/topics` returned 50 (positive control; `topic_writer.py` deliberately writes the DEFAULT graph). SAME missing scope in the GDPR Art. 17 erasure: a bare `DELETE ... WHERE` cannot reach a named graph, so `POST /api/v1/people/{slug}/forget` returned `forgotten:true, stores_purged:['oxigraph','qdrant']` and left the person's named-graph triples in place. Gate: `tests/test_a_forget_must_reach_every_named_graph.py`, wired by `.github/workflows/forget-reaches-every-graph.yml` | PR #1972 (`fix/forget-must-reach-every-named-graph`, CM051). ONE shared helper in `identity_resolver/compartment.py` (`cm048_user_graph_uris` + `graph_scoped_select`) rather than a per-surface patch: `topic_writer.py` and `last_contact_updater.py` each hit this trap, documented it in their own docstring and fixed only themselves, which is how it stayed systemic. The rewriter names this user's graphs EXPLICITLY and never emits a bare `GRAPH ?g`, which would put every other user's compartment on the primary's read path and re-create the leak `compartment.assert_default_graph_isolated` exists to prevent | **verified -- defect found and fixed, proven on a live Hub by round trip.** Forget: a synthetic person seeded 4 triples in each graph reached 0 in BOTH after the fix, while a second synthetic person was untouched at 4/4 (precision control). Readers: commitments 0 -> 50; person pages showing warmth/trust 0/15 -> 15/15. Mutation: reverting the two files on the box put all three back (50 -> 0, 15/15 -> 0/15, named graph survives the forget), with topics steady at 50 throughout as the control. Awaiting review; NOT merged |
