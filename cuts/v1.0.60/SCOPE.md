...
| G2 | Map all 19 surfaces to recorder call sites | TODO | **stated as UNMAPPED rather than assumed.** Recorder call sites: ok 14 · error 13 · no_data 9 · **cannot_run 5**  **MEASURED 2026-09-02 (Archie), still TODO:** the mapping is 19 surfaces -> **ZERO** recorder call sites. `git grep -ln` for record_source / source_status / sources_record / record_ingest across the tracked tree returns 0 files. Positive control: OSTLER_FDA_SOURCES returns 20 matches in install.sh, so the reader is alive and the zero is a real absence, not a broken search. Recorded so the next agent does not re-derive it: G is GREENFIELD -- no recorder, no record, no endpoint (G3: /api/v1/sources appears once in the tree and that once is this file asking for it), no panel, no consumers. Confirms task #269. |
