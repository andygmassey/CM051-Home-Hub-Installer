...
| A5 | `progress "Reading your Apple Notes"` (install.sh:25560) fires only when work happens | **DONE at source** (Archie, 2026-09-02) | install.sh:25693 (NOT :25560 -- the row's line number was stale too). The progress call sits in the `else` of `if [[ ! -s "$_HYDRATE_APPLENOTES_JSON_FILE" ]]`, so it fires ONLY when the staged JSON exists and is non-empty, i.e. only when work follows. The absent-data branch decrements TOTAL_STEPS so the bar does not reserve a slot for a step that never runs. The code comments the choice: the data gate is deliberately the ONLY announce condition, because gating on all four downstream branches would swap one silence for another. Shipped-artefact proof still owed on the walk. |
...
| # | Item | State | Proof required |
|---|---|---|---|
| E1 | Wire the T+0 probe subset into the end of install.sh | **BUILT + TEST-PROVEN, WALK-PROOF OWED** (A2 #1351 `d16e9402`; re-measured by Archie 2026-09-02) | ⚠️ **NOT MARKED DONE, AND THIS ROW'S OWN PROOF TEXT IS WHY.** It demands *"probe output in the install log on a real install"*, and **no install has run: 0 install logs on this machine.** What IS proved: the readout is on main (`Source status` 2 hits in install.sh), and `tests/test_e1_source_readout_emits.sh` passes **6 of 6** against the REAL E1 block with a stubbed curl, covering served / unreachable / 404. Its **anti-vacuity limb reverts `info` to `log` and requires NOTHING to emit**, so the green distinguishes emit from silent no-op rather than asserting it. A2 also found the sharper fact: pre-fix, `log "..."` was not silence, it invoked `/usr/bin/log` with a garbage subcommand. **A passing unit test is not an install log.** This row closes on the v1.0.60 walk, not before |
...
| # | Item | State | Proof required |
|---|---|---|---|
| G1a | `last_update_at`, DISTINCT from `recorded_at` | **DONE** (A2, merged `b38af35d`; verified by Archie 2026-09-02 at `4890c26e`) | `tests/test_source_status_contract.sh` rc=0: *"reader reads last_update_at the writer wrote (G1a)"* and separately *"reader reads recorded_at the writer wrote"*. Two fields, two assertions, both round-tripped. **Not a fixture:** the test reads the writer out of `install.sh` and the reader out of `vendor/doctor/agent/web_ui.py`, i.e. the VENDORED shipping copy, and CANNOT-RUNs (exit 2) if either half is missing. |
| G1b | typed `item_count` (today `payload` is a free string, `sent=0`) | **DONE** (A2; verified by Archie) | same run: *"reader reads the writer's item_count as typed int 42 (got 42)"* and *"no_data round-trips item_count 0 (got 0)"*. The zero case is the one that matters — a typed 0 is distinguishable from absent, where a free string was not. |
| G1c | **Nothing publishes the record** — no `/api/v1/sources`, so panel, probe and walk each infer state independently | **DONE** (A2; verified by Archie) | `@app.get("/api/v1/sources")` at `vendor/doctor/agent/web_ui.py:2433` — in the **VENDORED** tree, which is the half that ships, not merely at source. The old proof note on this row ("0 occurrences in ical-server") was correct when written and is now stale: the endpoint was built in the Doctor, not in ical-server. |
| G2 | Map all 13 surfaces to recorder call sites | **DONE** (A2, merged `b38af35d`) — **AND MY OWN EARLIER MEASUREMENT ON THIS ROW IS NOW STALE, CORRECTED BELOW** | **RE-MEASURED 2026-09-02T19:5xZ by Archie at `4890c26e`, with the identical commands that produced the zero:** `record_source` 1 file · `source_status` **7 files** · `sources_record` 1 · `record_ingest` 1, against the same positive control `OSTLER_FDA_SOURCES` (7 files). The recorders now exist. **The old note below was TRUE WHEN TAKEN and is FALSE NOW** — it is kept, struck, because a measurement that decayed is worth more as a dated record than as a deletion. Also corrected: the surface count is **13, not 19** (see G3). ~~**MEASURED 2026-09-02 (Archie), still TODO:** the mapping is 19 surfaces -> **ZERO** recorder call sites. `git grep -ln` for record_source / source_status / sources_record / record_ingest across the tracked tree returns 0 files. Positive control: OSTLER_FDA_SOURCES returns 20 matches in install.sh, so the reader is alive and the zero is a real absence, not a broken search. Recorded so the next agent does not re-derive it: G is GREENFIELD -- no recorder, no record, no endpoint (G3: /api/v1/sources appears once in the tree and that once is this file asking for it), no panel, no consumers. Confirms task #269.~~ |
| G3 | `/api/v1/sources` serves the record | **DONE at source + VENDORED** (A2; verified by Archie 2026-09-02) | ⚠️ **THIS ROW'S PROOF CRITERION WAS WRONG AND IS CORRECTED HERE.** It demanded *"200 with 19 rows"*. **There are 13, not 19.** The 19 was never measured — it is the same unmeasured number task #269 already flagged. `tests/test_source_status_canonical_matches_recorders.sh` rc=0 prints BOTH sets in full and asserts set equality: *"the served set is EXACTLY the recorder set (13 sources); no phantom, no missing"* — `ai_conversations apple_notes browsing calendar contacts dedupe email email_preferences imessage people places privacy_backfill whatsapp`. A number in a proof criterion is a claim like any other; this one had no measurement behind it. **Still owed: the 200 itself, on a box** — set equality in the tree is not a live response. |
| G4 | Panel renders landed + last-updated, and says WHY when dark | TODO | on a box |
| G5 | Probe (E1) and walk READ this artefact rather than re-implementing | **BUILT + TEST-PROVEN, WALK-PROOF OWED** (A2 #1351 `d16e9402`; re-measured by Archie 2026-09-02) | ⚠️ **SAME CORRECTION AS E1: built and test-proven is not walked.** A2 reported this TODO -> DONE and the mechanism half is right, but the row's proof is behavioural. Landed and measured: `/api/v1/sources` present in `vendor/doctor/agent/web_ui.py`, the probe row `source_status_artefact_is_served` declared in `cut-manifests/permanent.yaml` (runner-discovery is not cut-declaration, which is what `manifest-tests` caught), and the E1 emission test green 6 of 6 with its anti-vacuity limb firing. Still owed: the probe and the walk READING it on a real box. one artefact, three readers. Taken together with E1 because a probe that READS the endpoint proves the artefact is USED, not merely served, and because store-derived probes cannot see the "two empty stores agreeing on zero" case the record exists to expose |
