...
if [[ "$OSTLER_REGION" == "eu" ]]; then
    ...
    echo "    - Voice – labelling *who* is speaking on calls (speaker"
    echo "      identification). This is done only by the optional"
    echo "      Ostler iPhone Companion app, on your iPhone, under a"
    echo "      separate consent screen in that app – not on this"
    echo "      Mac. This Mac never receives voice recordings or"
    echo "      voice fingerprints. (We do not infer mood, emotion,"
    echo "      sentiment, stress or deception from voice.)"
    ...
    echo ""
    echo "  The categories above are processed on this Mac and stay on"
    echo "  it – the one exception is voice speaker-identification,"
    echo "  which happens only on your iPhone and never reaches this"
    echo "  Mac. None of it is sent to Creative Machines or any third"
    echo "  party, except in the specific cases listed in our Privacy"
    echo "  Policy at ostler.ai/privacy."
    echo ""
...
services:
  ...
  wiki-site:
    image: ghcr.io/ostler-ai/ostler-wiki-site@sha256:89a1578699dbf75ea277a0432270720e0fb1290e3dafe3d04e47222e4750367b
  ...
  wiki-compiler:
    image: ghcr.io/ostler-ai/ostler-wiki-compiler@sha256:538afd52743471956c6fa573b697de20ca0dd380874ebf24b300badb193d8e2b
...
progress "Setting up ostler-assistant binary (v${OSTLER_ASSISTANT_VERSION:-0.4.30})" "ostler_assistant"

OSTLER_ASSISTANT_VERSION="${OSTLER_ASSISTANT_VERSION:-0.4.30}"
