...
if [ -z "$pre" ] || [ -z "$rst" ] || [ -z "$stg" ]; then
    echo "CANNOT-RUN: could not locate all three section rules (preflight=${pre:-?} reset=${rst:-?} stage=${stg:-?})." >&2
...
if [ "$pre" -lt "$rst" ] && [ "$pre" -lt "$stg" ]; then
    ...
    bad "the preflight is at ${pre}, RESET at ${rst}, STAGE at ${stg}. A walk that cannot start must not first tear a box down."
fi
