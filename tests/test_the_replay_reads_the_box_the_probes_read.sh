...
echo "== 1b. nothing in the lib can invert under the caller's pipefail =="
# The whole file, not one function: the lib sets no options and inherits
# pipefail from run_box_walk.sh, so the pipefail ratchet (whose population is
# files that set pipefail themselves) never sees it. This arm is the only
# guard the file has. grep -c prints 0 and exits 1 on no match; || true keeps
# the single value (|| echo 0 would append a second zero).
n_pipes="$(grep -cE '\| *grep -q' "$LIB" || true)"
if [ "${n_pipes}" -eq 0 ]; then ok "no pipe into grep -q anywhere in wiki_summaries_wait.sh (a match must not read as could-not-run)"; else bad "wiki_summaries_wait.sh pipes into grep -q ${n_pipes}x; under run_box_walk.sh's pipefail a MATCH can read as could-not-run"; fi
