...
MARKERS=("_url(" "pw-band-head" "pw-heat")
...
# POSITIVE CONTROL for the marker loop above.
#
# The diff positive control at the foot of this file proves the BYTE
# comparison bites. It says nothing about this loop -- and the two answer
# different questions. The diff check fails when git and image disagree; the
# marker check is what still fails when they agree because BOTH lost the fix
# (someone reverts on main, rebuilds, re-pins -- byte-identical, and the
# design system is gone). That is the case this loop exists for, so it needs
# its own proof that it can fail.
#
# Probe: strip every marker from a copy and assert the same grep now misses.
# Caught by Archie on PR #529: a check with no proof it can fail is
# indistinguishable from a check that always passes.
PROBE="$SCRATCH/marker_probe.py"
cp "$SCRATCH/image_dashboard.py" "$PROBE"
for marker in "${MARKERS[@]}"; do
    # -F: markers contain regex metacharacters ("_url(" has an open paren).
    grep -vF -- "$marker" "$PROBE" > "$PROBE.tmp" && mv "$PROBE.tmp" "$PROBE"
    grep -q -- "$marker" "$PROBE" && fail "POSITIVE CONTROL FAILED -- '$marker'
      still matches after every line containing it was removed. The marker
      check cannot detect a stale generator."
done
