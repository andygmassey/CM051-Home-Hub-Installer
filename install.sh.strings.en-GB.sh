...
MSG_FAIL_FDA_DEPENDENCIES_IMPORT_RE_RUN="The data-extraction module was installed but cannot be loaded, so the background refresh would fail every time it ran. Full output saved to /tmp/ostler-fda-deps.log – re-run the installer, and attach that file if you email support@ostler.ai (Reference: ERR-10-FDA-DEPS-IMPORT)."
