...
i.e. it carries: id, subject, domain, category, polarity, privacy, score,
...
markers). ``privacy`` is the producer half of the external-scout egress
contract (scout_external fail-closes on it): only L0/L1 records may ever
feed an outbound query, and any record reaching this emitter without a
recognisable level is stamped L3 here so the artefact contract is total.
``compile_profile`` groups likes by domain and splits dislikes
...
def build_artefact(profile: dict, now: datetime | None = None) -> dict:
    ...
    Backstop on the egress contract: every record in the artefact carries a
    recognised ``privacy`` level. A profile compiled by an older build (or a
    record with a mangled tag) is stamped L3 - explicit and most restrictive
    - rather than shipped untagged. Records are copied, not mutated."""
    now = now or datetime.now(timezone.utc)
    interests = []
    for it in flatten_profile(profile):
        if ip.normalise_privacy(it.get("privacy")) is None:
            it = dict(it)
            it["privacy"] = "L3"  # fail-closed: cannot prove it is safe
        interests.append(it)
    return {
