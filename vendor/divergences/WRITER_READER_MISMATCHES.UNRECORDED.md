...
## Added 2026-09-19, CM051 #2233 -- `ostler_fda`, the knowledge import graft

**Location and shape only, never content.**

### What diverges

`vendor/ostler_fda/universal_import.py`, and nothing else in the tree.

HR015 #977, sha 28662818, grafted here. The dispatcher ran
`ostler-knowledge convert` and stopped. Convert writes markdown and indexes
nothing, so a customer who dropped a Notion or Obsidian export got files on disk
that nothing could find, and the import reported ok.

### Why this entry is unusual, and why the record is still owed

The graft makes the vendored file **byte-identical to upstream at #977**, so for
this file the tree now MATCHES upstream rather than departing from it. Measured:
vendored was byte-identical to upstream at the pin (0 differing lines, self-diff
control 0); upstream at #977 differs from both by exactly 172 lines, which is
this commit and nothing else; after the graft the vendored file and upstream at
#977 are identical.

The divergence is therefore **against the PIN**, not against upstream, and it
retires the moment the pin moves to #977 or beyond.

### The refusal, and it was MINE rather than the tool's

`scripts/regenerate_divergence_patch.sh ostler_fda --check` was run against an
HR015 worktree checked out at exactly the pinned sha. **It did not refuse.** It
proposed a patch, and the proposal is why it was not written:

    files touched : 12
    lines added   : 2691
    lines removed : 74

That is the tree's accumulated undescribed divergence, not this graft. The
tool's own words are that every recorded line becomes a permanent claim the
difference is DELIBERATE, and that accidental drift must be fixed in the tree
rather than recorded. Twelve files and ~2,700 lines have not been reviewed by
anyone in this change, so recording them would convert unreviewed drift into
declared intent with one command.

Its PII scan is also explicit about its own limit: 3,099 changed lines scanned,
both signs, no hits, **numeric shapes only**, and it states that it cannot see a
person name and that for names the answer is UNKNOWN rather than clean.

### What is owed

1. Review the ~2,700 lines of pre-existing `ostler_fda` divergence and either
   fix the tree or record it deliberately. That is a real finding surfaced by
   this graft and it is larger than the graft.
2. Move the pin to #977 or beyond, at which point this entry retires by itself.

Neither is done, and this file exists to say so.
