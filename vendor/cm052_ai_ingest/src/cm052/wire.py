...
def _privacy_level(conv: Conversation) -> str:
    """Read the privacy classification off Conversation metadata.

    AI conversations default to **L3** ("private by default"). The
    rationale is that AI chat content is typically denser-than-
    human-chat in user secrets -- repo internals, API keys, security
    audit notes, financial context, partner names -- so the
    synced-contacts default of L2 underplays the sensitivity. L3 lets
    the wiki acknowledge the conversation exists (date, source_app,
    message count, slug) without rendering body content on a casual
    browse, and a per-conversation override (``privacy_level: L1``
    in metadata) lets the operator dial *down* sensitivity for
    conversations they want indexed and searchable.

    L0..L3 are all emitted; the consumer side (CM044 renderer, MCP
    clients) decides what to do with each level. Unrecognised values
    fall back to the L3 default rather than the lowest level so a
    typo cannot accidentally publish private content.
    """
    raw = (
    ...
    if isinstance(raw, str) and raw.upper() in ("L0", "L1", "L2", "L3"):
        return raw.upper()
    return "L3"
...
def render_episodic(conv: Conversation, *, user_display: str | None = None) -> str:
    ...
    fm: dict = {
        ...
        "privacy_level": _privacy_level(conv),
