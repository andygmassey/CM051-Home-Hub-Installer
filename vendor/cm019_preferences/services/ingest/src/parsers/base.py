...
# Stable namespace for deterministic *document* ids. A "document" is the source
# artefact a batch of preferences was extracted from (e.g. a Spotify export
# file). All points from one document share a ``doc_id`` so that a single
# document can be deleted/replaced in Qdrant without rebuilding the collection
# (REUSE-3 per-document vector delete). Derived once from a fixed DNS name so
# the value never changes between runs or processes.
_DOCUMENT_ID_NAMESPACE = uuid.uuid5(uuid.NAMESPACE_DNS, "document.pwg.ostler")


def derive_doc_id(source: str, document_ref: str) -> str:
    """Derive a stable ``doc_id`` for a source document.

    ``source`` is the parser/source name (e.g. ``"spotify"``) and
    ``document_ref`` is a per-source-stable reference to the document, normally
    its file path (or any caller-chosen stable handle). The same logical
    document always maps to the same id, so re-ingesting it after a correction
    deletes/replaces exactly that document's points rather than duplicating
    them. A UUIDv5 keeps the value a valid string usable as a Qdrant payload
    value and filter target.
    """
    key = f"{source}|{document_ref}"
    return str(uuid.uuid5(_DOCUMENT_ID_NAMESPACE, key))
...
class ParsedPreference:
    ...
    # Stable id of the source *document* this preference came from. Set by the
    # pipeline (per file) so every point from one document shares it, enabling
    # per-document delete/replace in Qdrant. Left None for ad-hoc preferences
    # that have no document of origin.
