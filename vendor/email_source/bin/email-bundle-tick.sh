...
_ostler_watermark="${OSTLER_STATE_DIR:-$HOME/.ostler/workspace}/email_source_state.json"
...
if ! mkdir "$_ostler_lock" 2>/dev/null; then
    ...
    if [ -n "${_ostler_h:-}" ] && kill -0 "$_ostler_h" 2>/dev/null; then
        if [ "$_ostler_never_ran" = "1" ] && [ "$_ostler_starve_wait" -gt 0 ]; then
            echo "email-bundle tick: slot held by pid ${_ostler_h}; this feed has never run, waiting up to ${_ostler_starve_wait}s for a turn."
            _ostler_waited=0
            _ostler_got_lock=0
            ...
            if [ "$_ostler_got_lock" != "1" ]; then
                ...
                echo "email-bundle tick: slot still busy after waiting ${_ostler_waited}s; yielding this tick (next tick retries)."
                exit 0
            fi
        else
...
"$PYTHON_BIN" -m email_source.pipeline "${ARGS[@]}"
