...
# drains at full throttle. Nothing is lost: the per-thread watermark
# means an older thread deferred during the day is picked up on the
# next overnight tick.
...
if [ "$_ostler_slot_active" = "1" ]; then
    ...
    ostler_slot_acquire "email-bundle" || exit 0
...
# exactly one of the eleven named surfaces did. This is email conversation capture.
#
...
if [ -f "$_ostler_gate" ]; then
    ...
    if [ "$_ostler_gate_rc" -eq 3 ]; then
        echo "email-bundle tick: Ostler Pro is not active, so new email conversation capture is paused. Everything already in your Hub stays available. Subscribe in the Ostler app and this resumes on the next tick."
        exit 0
    fi
    if [ "$_ostler_gate_rc" -ne 0 ]; then
        echo "email-bundle tick: subscription gate check exited ${_ostler_gate_rc} (expected 0 or 3); continuing to ingest. A customer is never paused because we could not ask." >&2
    fi
else
    echo "email-bundle tick: subscription gate not found at ${_ostler_gate}; continuing to ingest. Rule 0.8 is NOT being enforced on this surface -- this line is the only evidence of that, so do not remove it." >&2
fi
# --- end Rule 0.8 gate -----------------------------------------------
...
_ostler_run "$PYTHON_BIN" -m email_source.pipeline "${ARGS[@]}"
