...
markdown-writer frontmatter::
    ...
    evernote_guid -> ParsedNote.evernote_guid (stable per-note id; idempotent re-ingest)
    ...
    The stable id rides under ``evernote_guid`` -- the ostler_fda writer
    ``pop()``s its internal ``note_id`` and re-emits it under the
    CM024-generic ``evernote_guid`` key (the field the Notion adapter also
    repurposes). ``_record_id`` reads ``evernote_guid`` first, falling back
    to ``note_id`` so legacy/synthetic ``note_id``-keyed input still maps.
...
def _record_id(record: dict) -> Optional[str]:
    """Stable per-note id from a record, tolerant of the writer's key name.

    The HR015 ``ostler_fda`` Apple Notes parser surfaces the note's stable
    id under ``evernote_guid`` (it ``pop()``s the internal ``note_id`` and
    re-emits it under the CM024-generic ``evernote_guid`` key, the same one
    the Notion adapter repurposes). We read ``evernote_guid`` first and fall
    back to ``note_id`` so both the real export and any legacy/synthetic
    ``note_id``-keyed input map to a non-null id -- keying dedup on this is
    what makes a timer-driven re-scan idempotent.
    """
    return record.get("evernote_guid") or record.get("note_id")
...
class AppleNotesAdapter:
    ...
    def parse(self, raw: RawNote) -> Optional[ParsedNote]:
        ...
        return ParsedNote(
            ...
            tags=tags,
            evernote_guid=_record_id(record),  # stable id for idempotent re-ingest
        )
