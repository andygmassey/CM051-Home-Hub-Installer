"""Doctor endpoint logic for the Obsidian import service (CM024 Knowledge).
...
UI exposes three routes that this module backs:
    ...
    POST /api/v1/import/obsidian                – start a job
    GET  /api/v1/import/obsidian/<job_id>/status – poll job state
    GET  /api/v1/import/obsidian/<job_id>/tail   – tail the last 100 log lines

All four customer-visible surfaces (3 API routes + the ``/import-obsidian``
UI page) are feature-flag-gated by ``features.obsidian_import`` in
...
config directory -- NOT a single file. So the only real divergence from
the Evernote module is ``validate_obsidian_path`` (requires a directory
that contains ``.obsidian/``, mirroring the CM024 adapter's own guard).
Convert + embed run via the same shared runner, and the embed phase
carries the same L3 privacy cap (``--max-compartment-level``).

Concurrency: a single global lockfile at
``~/.ostler/locks/import-obsidian.lock`` prevents two imports running
...
* While running, only ``~/.ostler/locks/import-obsidian.lock`` exists.
...
* On completion (or runner crash), the runner wrapper writes a
  ...
  ``~/.ostler/state/import-obsidian-<job_id>.json`` with status =
...
shared-runner contract (``--enex-path``); for Obsidian it carries the
vault path. Renaming is deferred to the eventual shared-core refactor.
"""
...
LOCK_FILENAME = "import-obsidian.lock"
LOG_FILENAME_PREFIX = "import-obsidian-"
STATE_FILENAME_PREFIX = "import-obsidian-"
...
def is_feature_enabled(*, _path: Optional[Path] = None) -> bool:
    """Return True iff ``features.obsidian_import: true`` in features.yaml.
    ...
    if not isinstance(features, dict):
        return False
    return features.get("obsidian_import") is True


# ── Path validation ───────────────────────────────────────────────────


def validate_obsidian_path(raw_path: Any) -> Path:
    """Resolve and validate an Obsidian vault path supplied by the user.

    An Obsidian vault is a directory containing a ``.obsidian/`` config
    directory. We require both -- a plain folder of markdown is NOT a
    vault, and refusing it here mirrors the CM024 ``obsidian`` adapter's
    own guard (so the operator gets a clear 400 up front rather than a
    cryptic convert failure later). Returns the resolved absolute Path.
    Raises ``ObsidianImportError(400)`` on any failure -- missing field,
    wrong type, empty string, unresolvable path, not a directory, or a
    directory with no ``.obsidian/``.

    Path validation is light-touch on location: we resolve symlinks so
    the operator can paste a symlinked path, and we do **not** restrict
    the vault to a particular root (it legitimately lives anywhere).
    """
    if not isinstance(raw_path, str):
        raise ObsidianImportError(400, "vault_path must be a string")

    stripped = raw_path.strip()
    if not stripped:
        raise ObsidianImportError(400, "vault_path must be non-empty")

    try:
        resolved = Path(stripped).expanduser().resolve()
    except (OSError, RuntimeError) as exc:
        raise ObsidianImportError(400, f"could not resolve vault_path: {exc}")

    if not resolved.is_dir():
        raise ObsidianImportError(
            400, f"Obsidian import expects a vault directory: {resolved}",
        )

    if not (resolved / ".obsidian").is_dir():
        raise ObsidianImportError(
            400,
            "not an Obsidian vault (no .obsidian/ config directory): "
            f"{resolved}",
        )
...
def start_import(
    ...
    *,
    ...
    ``ObsidianImportError(409)`` if a non-stale lock is held by another
...
def _safe_job_id(job_id: Any) -> str:
    ...
    if not isinstance(job_id, str) or not _JOB_ID_RE.match(job_id):
        raise ObsidianImportError(400, "invalid job_id")
    return job_id
...
def read_status(
    ...
    *,
    ...
    raise ObsidianImportError(404, f"unknown job_id: {job_id}")
...
def read_tail(
    ...
    *,
    ...
    try:
        ...
        raise ObsidianImportError(500, f"could not read log: {exc}")
