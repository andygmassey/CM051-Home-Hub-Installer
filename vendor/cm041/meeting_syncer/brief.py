...
# Canonical fail-closed L3 privacy helper -- the SAME module the Hub
# server (assistant_api/ical-server.py) uses. _PARENT_DIR (repo root) is
# already on sys.path above, so this resolves to <repo-root>/pwg_privacy.py.
# VENDORING: if this file is vendored downstream, pwg_privacy.py must go
# with it.
import pwg_privacy
...
# Owner labels that denote the OPERATOR'S OWN calendar (mirrors the write
# side in contact_syncer.google_calendar). A calendar fact whose
# pwg:sourceCalendar owner is NOT one of these - i.e. a distinct third
# party - must never be returned as this person's own fact.
_OPERATOR_OWNER_TOKENS = frozenset(
    {"you", "me", "self", "myself", "my calendar", "operator"}
)


def _owner_is_operator(owner):
    """True when a calendar-owner label denotes the operator themselves."""
...
def _get_person_facts(oxigraph_url, person_uri):
    """Get current facts about a person, L3-filtered and owner-filtered.

    Fail-closed L3 filter (operator doctrine, Archie batch-1 F8), routed
    through the SAME shared ``pwg_privacy`` helper the Hub server uses so
    the two readers cannot drift. Each fact's own ``pwg:privacyLevel`` and
    the owning person's ``pwg:privacyLevel`` are both selected;
    most-restrictive wins. An untagged fact on an L2 person stays (this is
    an attendee-brief surface, i.e. L2 by nature); an L3 fact, or any fact
    whose owning person is L3, or a fact with no parseable clearance on
    either axis, is dropped.

    Orthogonally, calendar-domain facts carry ``pwg:sourceCalendar`` (whose
    diary the event came from). A calendar fact whose owner is a DISTINCT
    third party must not be returned as this person's own fact - that is the
    partner-diary attribution leak (BATCH1 #3). The write side now withholds
    ``pwg:aboutPerson`` from non-operator calendars, but legacy facts
    written before that fix still carry the operator stamp; this reader
    filters them defensively by ``pwg:sourceCalendar``. This owner filter is
    orthogonal to the L3 filter - a partner calendar can be perfectly L1 and
    still be mis-attributed - so both filters apply.
    """
    sparql = """
    PREFIX pwg: <{ns}>
    SELECT ?text ?domain ?fpriv ?ownerPriv ?owner WHERE {{
      ?fact a pwg:PersonFact ;
            pwg:aboutPerson <{uri}> ;
            pwg:factText ?text .
      OPTIONAL {{ ?fact pwg:factDomain ?domain }}
      OPTIONAL {{ ?fact pwg:privacyLevel ?fpriv }}
      OPTIONAL {{ <{uri}> pwg:privacyLevel ?ownerPriv }}
      OPTIONAL {{ ?fact pwg:sourceCalendar ?owner }}
      FILTER NOT EXISTS {{ ?fact pwg:validTo ?end }}
    }}
    """.format(ns=PWG, uri=person_uri)
    result = _sparql_query(oxigraph_url, sparql)
    facts = []
    for b in result.get("results", {}).get("bindings", []):
        fact_level = b.get("fpriv", {}).get("value")
        owner_level = b.get("ownerPriv", {}).get("value")
        if pwg_privacy.is_l3({"privacy_level": fact_level},
                             owner_level=owner_level):
            continue
        domain = b.get("domain", {}).get("value", "")
        owner = b.get("owner", {}).get("value", "")
        if domain == "calendar" and owner and not _owner_is_operator(owner):
            # A calendar event owned by a distinct third party: never
            # surface it as this person's own fact (partner-diary leak).
            continue
