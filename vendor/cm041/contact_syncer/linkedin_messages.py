...
def group_conversations(rows: List[Dict[str, str]], user_name: str) -> Dict[str, Dict]:
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Your name as it appears in the FROM field. Falls back to the "
            "USER_DISPLAY_NAME (or PWG_USER_NAME) env var. Required — no "
            "hardcoded default, so another user's export can't get silently "
            "tagged under the developer's identity."
        ),
    )
    ...
    if not args.user_name:
        print(
            ...
            "hardcoded name because it would silently misattribute your "
            "messages.",
            file=sys.stderr,
        )
        return 2
