...
Each record also carries a stable per-note identifier under
``evernote_guid`` -- the field CM024's ``ParsedNote`` uses as its
generic source id (the Notion adapter already repurposes it the same
way). We populate it from the note's ``ZIDENTIFIER`` (a per-note UUID
that survives iCloud sync), falling back to ``apple_notes:<Z_PK>``
only if a row somehow lacks one. This makes a timer-driven re-scan
idempotent: the same note yields the same id, so the knowledge ingest
...
Incremental ("refresh since last run") mode
============================================

Apple Notes is a LIVE-LOCAL pollable source in the #595 topped-up
ingest design, so it can ride the existing ~15-min launchd timer.
Passing ``since`` (Python) / ``--since <ISO-8601>`` (CLI) filters the
SQL to notes with a modification date strictly newer than the prior
watermark, so a re-scan only re-emits what changed instead of all
~1000 notes every tick. The natural watermark column is
``ZICCLOUDSYNCINGOBJECT.ZMODIFICATIONDATE1`` (Core-Data seconds since
2001-01-01, the same units ``_mac_to_datetime`` already converts). The
run reports a ``watermark`` = the max modification date seen (ISO-8601)
which the caller persists (the design's ``~/.ostler/state/<source>.json``)
and feeds back as ``--since`` next tick. Absent ``--since`` = a full
scan, so the install-time hydrate is unchanged.

  * Null / unparseable modification dates: such notes are KEPT in
    incremental mode (the ``OR ... IS NULL`` arm of the predicate), so
    a row with a missing date is never silently skipped between ticks.
    They re-emit every run, which stays idempotent downstream via the
    stable note id, and they do not contribute a watermark candidate.
  * Empty incremental run (zero notes newer than ``since``): the
    watermark passes the input ``since`` THROUGH unchanged so it never
    regresses to null.
  * Deleted notes are OUT OF SCOPE here -- removing a note that
    vanished from the store is the deletion-reconcile v1.1 item in the
    #595 design, not this incremental-add pass. (Notes flagged
    ``ZMARKEDFORDELETION`` are already excluded by the base query.)
...
def _iso_to_mac(iso: Optional[str]) -> Optional[float]:
    """Convert an ISO-8601 timestamp string to a Core-Data absolute time.

    Inverse of ``_mac_to_datetime`` -- used to turn an incremental
    ``--since`` watermark back into the seconds-since-2001 units the
    ``ZMODIFICATIONDATE1`` column is stored in, so the comparison can
    run SQL-side. A bare date or a naive datetime is treated as UTC.

    Returns None for an empty / unparseable value (the caller then
    treats the run as a full scan rather than guess a cutoff).
    """
    if not iso:
        return None
    raw = iso.strip()
...
class Note:
    ...
    note_id: str            # stable per-note id (-> evernote_guid on output)
    title: str
...
def _note_to_record(n: Note) -> dict:
    ...
    The stable ``note_id`` is surfaced under ``evernote_guid`` -- the
    field CM024's ``ParsedNote`` / markdown-writer frontmatter uses as
    its generic source id. Keying dedup on this is what makes a
    re-scan idempotent (update-in-place rather than re-insert).
    """
    d = asdict(n)
    d["evernote_guid"] = d.pop("note_id")
...
#
# ``{since_clause}`` is filled with an empty string for a full scan, or
# an incremental predicate (see ``_since_clause``) when a ``--since``
# watermark is supplied. The predicate keeps null-dated notes so a row
# with an unparseable / missing modification date is never silently
...
# Incremental predicate appended to the real-schema query when a
# ``since`` watermark is supplied. Filters on the same modification
# column the watermark is derived from. Null-dated notes are KEPT
# (``IS NULL``) so a row with a missing/unparseable date is never
# silently skipped between ticks -- it re-emits each run, which stays
# idempotent downstream via the stable note id.
_SINCE_CLAUSE_REAL = (
    "AND (COALESCE(n.ZMODIFICATIONDATE1, n.ZMODIFICATIONDATE, "
    "n.ZCREATIONDATE3, n.ZCREATIONDATE1) > :since_mac "
    "OR COALESCE(n.ZMODIFICATIONDATE1, n.ZMODIFICATIONDATE, "
    "n.ZCREATIONDATE3, n.ZCREATIONDATE1) IS NULL)"
)
...
# Incremental predicate for the legacy schema (un-suffixed column).
_SINCE_CLAUSE_LEGACY = (
    "AND (n.ZMODIFICATIONDATE > :since_mac "
    "OR n.ZMODIFICATIONDATE IS NULL)"
)
...
def extract_notes(
    ...
    """Extract notes from Apple Notes as KNOWLEDGE records.
    ...
    Args:
        ...
        since: Optional ISO-8601 watermark. When supplied, only notes
            with a modification date strictly newer than ``since`` are
            returned (INCREMENTAL mode), so a timer-driven re-scan only
            re-emits what changed. Notes whose modification date is
            null/unparseable are KEPT (never silently skipped). An
            empty/unparseable ``since`` falls back to a FULL scan
            (the unchanged default). Compute the next watermark with
            ``watermark_for_notes(notes, since)``.
    ...
    since_mac = _iso_to_mac(since)
    if since and since_mac is None:
        logger.warning(
            "Apple Notes --since value not parseable as ISO-8601; "
            "running a full scan instead"
        )
    incremental = since_mac is not None
    params = {"since_mac": since_mac} if incremental else {}
    ...
    query_real = _QUERY_REAL.format(
        since_clause=_SINCE_CLAUSE_REAL if incremental else ""
    )
    query_legacy = _QUERY_LEGACY.format(
        since_clause=_SINCE_CLAUSE_LEGACY if incremental else ""
    )

    try:
        try:
            rows = conn.execute(query_real, params).fetchall()
        except sqlite3.OperationalError as e:
            logger.debug("Primary Apple Notes query failed (%s); trying legacy", e)
            try:
                rows = conn.execute(query_legacy, params).fetchall()
    ...
    for row in rows:
        ...
        # Stable per-note id for idempotent re-ingest. ZIDENTIFIER is a
        # per-note UUID that survives iCloud sync; fall back to the
        # Core-Data primary key (stable per local DB file) only if it
        # is somehow absent.
        identifier = (row["identifier"] or "").strip()
        note_id = identifier or f"apple_notes:{row['pk']}"
...
def watermark_for_notes(
    notes: List[Note], since: Optional[str] = None
) -> Optional[str]:
    """Compute the next incremental watermark to persist for the caller.

    The watermark is the maximum note modification date (``updated``)
    seen this run, as an ISO-8601 string. The caller persists it (the
    #595 design's ``~/.ostler/state/<source>.json``) and passes it back
    as ``--since`` next tick so only newer notes are re-emitted.

    If no notes were seen (zero matched an incremental query), the
    input ``since`` is passed THROUGH unchanged -- the watermark must
    never regress to null on an empty run, or the next tick would
    re-scan everything. Notes with a null modification date do not
    contribute a candidate (they re-emit every run by design); if every
    note seen is null-dated the watermark also falls back to ``since``.
    """
    candidates = [n.updated for n in notes if n.updated is not None]
    if not candidates:
        return since
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        metavar="ISO8601",
        help=(
            "Incremental watermark. Only emit notes modified strictly "
            "after this ISO-8601 timestamp (e.g. 2025-03-16T09:30:00+00:00). "
            "The emitted 'watermark' field is the max modification date "
            "seen -- persist it and pass it back next tick. Absent = "
            "full scan (default)."
        ),
    )
    parser.add_argument(
    ...
    result: dict = {
        ...
        "total_words": 0,
        "notebooks": 0,
        # Incremental high-water mark. On an incremental run this is the
        # max note modification date seen (ISO-8601); the caller persists
        # it and passes it back as --since next tick. Passes the input
        # --since through unchanged when zero notes match, so a quiet
        # tick never regresses the watermark. Null on a full scan with
        # no notes (and no input watermark).
        "watermark": args.since,
        "incremental": bool(args.since),
        "errors": [],
        "status": "ok",
    }
    ...
    result.update(stats)
    # Advance (or pass through) the incremental watermark.
    result["watermark"] = watermark_for_notes(notes, args.since)
