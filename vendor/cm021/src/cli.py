...
def _safe_person_iri(email: str) -> str:
    """Return a URL-safe Person IRI rooted at urn:pwg:person/.

    Uses ``urllib.parse.quote`` so addresses with ``+`` aliases,
    Unicode local parts, or dots round-trip cleanly. The IRI is
    deterministic so subsequent ticks UPSERT instead of creating
    duplicates.
    """
    return f"urn:pwg:person/{urllib.parse.quote(email.lower(), safe='')}"
...
def _build_upsert(
    ...
    parts.append("PREFIX pwg: <urn:pwg:>")
