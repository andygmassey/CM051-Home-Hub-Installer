...
missing it falls back to a `git clone` of an upstream repo. The clone
path is fine for developer machines (where credentials and network
access are reliable) but it makes the customer install non-hermetic
-- a private repo, a network blip, or a credentials prompt can all
silently turn the install into a half-install.
...
a sync (e.g. `chore(vendor): sync cm024_knowledge from evernote-knowledge @ <sha>`),
