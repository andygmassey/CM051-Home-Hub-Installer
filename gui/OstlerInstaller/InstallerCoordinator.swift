...
final class InstallerCoordinator: ObservableObject {
    ...
    //
    // X is a monotonic counter incremented on each PROMPT event so the
    // customer sees "Question 5 of 17" rather than guessing how many
    // taps remain. Y is computed when the `channel_choice` prompt is
    // answered (because the channel selection picks 1 of 4 expected
    // path lengths) and may be revised when later branches are
    // committed (e.g. opting into a custom IMAP server adds another
    // ~6 prompts). Until Y is known the header degrades to "Question X".
    ...
    struct AnsweredQuestion: Identifiable, Equatable {
        let index: Int          // The X value at the time of commit (1-based).
    ...
    /// server adds ~6 follow-up prompts). v7.1 ships approximate
    ...
    private func recomputeTotalQuestionCount(committedPromptId id: String, answer: String) {
        switch id {
        case "channel_choice":
