...
//     and that prompt was the Mac Studio retest #5 failure mode.
//
...
final class CaffeinateManager {
    ...
    func start() -> pid_t? {
        ...
        do {
            try proc.run()
            process = proc
            NSLog("CaffeinateManager: started caffeinate -dimsu pid=\(proc.processIdentifier)")
            return proc.processIdentifier
        } catch {
            NSLog("CaffeinateManager: launch failed -- \(error.localizedDescription)")
            return nil
        }
    }
    ...
    func stop() {
        ...
        if proc.isRunning {
            proc.terminate()
            NSLog("CaffeinateManager: terminated caffeinate pid=\(proc.processIdentifier)")
        }
        process = nil
    }
