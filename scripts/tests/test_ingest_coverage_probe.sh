...
elif ! printf '%s' "$OUT" | grep -q 'NO store credential'; then fail arm2-reason "CANNOT-RUN but reason does not say credential absent"
...
elif ! printf '%s' "$OUT" | grep -q 'no HTTP response'; then fail arm3-reason "CANNOT-RUN but reason not a transport failure"
