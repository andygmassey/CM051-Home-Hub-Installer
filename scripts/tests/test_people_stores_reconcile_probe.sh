...
# ARM 7 (residual b): AUTHFAIL must NAME which store refused, not say "a store".
# Oxigraph is hit first (the triples control), so its name must appear.
start_fake "$OK_PORT" 401
OUT="$(run_probe_capture "http://127.0.0.1:${OK_PORT}/query" "${WORK}/conf_b" auth)"; stop_fake
V="$(verdict_of "$OUT")"
if [ "$V" != "VERDICT: FAIL" ]; then fail arm7-store "got '${V}', expected FAIL"
elif [ "$(printf '%s' "$OUT" | grep -cF 'Oxigraph')" -eq 0 ]; then fail arm7-store-name "AUTHFAIL FAIL does not name the refusing store -- residual-b: 'a store' is not actionable"
else note "arm7 AUTHFAIL names the refusing store (Oxigraph) ✅"; fi
