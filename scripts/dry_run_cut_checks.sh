...
build-recovery|SKIP|builds and Developer-ID signs the standalone Recover Ostler.app (xcodebuild archive with CODE_SIGN_IDENTITY, then codesign --verify). Same class as `release` below and excluded for the same reason: this job imports no identity, so running it would exercise the unsigned branch and prove the wrong thing. NOTHING IS LOST TO TIER 2 HERE, and that was checked rather than assumed: its prerequisites are guard-local-cut (a tier-1 row above) plus check-tools, check-identity and check-pbxproj-sync, all three already tier-2 rows via `release`.
